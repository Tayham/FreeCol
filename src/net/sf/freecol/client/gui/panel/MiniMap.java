...
public final class MiniMap extends JPanel implements MouseInputListener {
	...
	/**
	 * Set tile size to the given value, or the minimum or maximum bound of the tile
	 * size.
	 *
	 * @param size
	...
	public void paintMap(Graphics graphics) {
		...
		/*
		 * xSize and ySize represent how many tiles can be represented on the mini map
		...
		/*
		 * Iterate through all the squares on the mini map and paint the tiles based on
		 * terrain
		 */
		...
		/*
		 * Defines where to draw the white rectangle on the mini map. miniRectX/Y are
		 * the center of the rectangle. Use miniRectWidth/Height / 2 to get the upper
		 * left corner. x/yTiles are the number of tiles that fit on the large map
		 */
		if (getParent() != null) {
	...
	private void focus(int x, int y) {
		...
		// When focusing out on the minimap, the last available focus out takes a larger
