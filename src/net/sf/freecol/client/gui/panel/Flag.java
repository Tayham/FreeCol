...
public class Flag {
	 ...
	 * triangles of different color. The alignment of the background influences the
	 * size of the canton, if there is one.
	 */
	 ...
	 * diagonal bars. The decoration limits the shape and possible positions of the
	 * "union".
	 */
	public enum Decoration {
		NONE(), CROSS(UnionPosition.CANTON), GREEK_CROSS(UnionPosition.CANTON), SCANDINAVIAN_CROSS(
				UnionPosition.CANTON), CHEVRON(UnionShape.CHEVRON, UnionPosition.LEFT), PALL(UnionShape.CHEVRON,
						UnionPosition.LEFT), BEND(UnionShape.BEND, UnionPosition.TOP,
								UnionPosition.BOTTOM), BEND_SINISTER(UnionShape.BEND, UnionPosition.TOP,
										UnionPosition.BOTTOM), SALTIRE(UnionShape.CHEVRON, UnionPosition.TOP,
												UnionPosition.BOTTOM, UnionPosition.LEFT,
												UnionPosition.RIGHT), SALTIRE_AND_CROSS(UnionPosition.CANTON);
	 ...
	 * background. The "canton" is the top left quarter of the flag. Other quarters
	 * might be added.
	 */
	public enum UnionPosition {
		LEFT(Alignment.VERTICAL, 0), CENTER(Alignment.VERTICAL, 1), RIGHT(Alignment.VERTICAL, 2), TOP(
				Alignment.HORIZONTAL, 0), MIDDLE(Alignment.HORIZONTAL,
						1), BOTTOM(Alignment.HORIZONTAL, 2), CANTON(Alignment.NONE, 0), NONE(null, 0);
	...
	/**
	 * The distribution of stars in the "union", based on historical flags of the
	 * United States. This really should be extended to handle rectangles of
	 * different shapes.
	 */
	 ...
	 * from top to bottom, or from left to right. If the background is quartered,
	 * the colors will be used clockwise, starting at the top left. If there are
	 * more colors than stripes or quarters, the superfluous colors will be ignored.
	 */
	 ...
	 * triangle if the given shape is BEND, and a small left triangle if the given
	 * shape is CHEVRON or TRIANGLE.
	 *
	 * @param unionShape
