...
public final class ColonyPanel extends PortPanel implements ActionListener, PropertyChangeListener {
	...
	private void initialize(Colony colony) {
		...
		add(unloadButton, "span, split " + buttonFields + ", align center");
		add(fillButton);
		add(warehouseButton);
	 ...
	 * secondary effect on another, and especially if the population hits a bonus
	 * boundary. A simple example is that adding extra lumber production may improve
	 * the production of the lumber mill. These changes can then flow on to
	 * production and construction displays.
	 */
	...
	/**
	 * Generates a menu containing the units currently accessible from the Colony
	 * Panel allowing keyboard access to said units.
	 */
	 ...
	 *
	 * Try to use this at the top of all the routines that need the colony, to get a
	 * stable value. There have been nasty races when the colony changes out from
	 * underneath us, and more may be lurking.
	 *
	...
	/**
	 * Gets the list of units on the colony tile. Note, does not include the units
	 * *inside* the colony.
	 *
	...
	/**
	 * A panel that holds UnitLabels that represent Units that are standing in front
	 * of a colony.
	 */
	...
	/**
	 * A panel that holds UnitLabels that represent naval Units that are waiting in
	 * the port of the colony.
	 */
	...
	public final class BuildingsPanel extends JPanel {
		...
		public final class ASingleBuildingPanel extends BuildingPanel implements DropTarget {
			...
			public int suggested(GoodsType type) {
				return -1;
			} /** N/A Interface PropertyChangeListener. */
	 ...
	 * TilesPanel class must use the ImageLibrary inside SwingGUI.tileMapViewer for
	 * everything.
	 */
	public final class TilesPanel extends JPanel {
		...
		/**
		 * Panel for visualizing a <code>ColonyTile</code>. The component itself is not
		 * visible, however the content of the component is (i.e. the people working and
		 * the production)
		 */
		public final class ASingleTilePanel extends JPanel implements DropTarget, PropertyChangeListener {
			...
			/**
			 * Updates the description label, which is a tooltip with the terrain type, road
			 * and plow indicator, if any.
			 *
