...
public class BuildQueuePanel extends FreeColPanel implements ItemListener {
	 ...
	 * <code>BuildQueueItem</code>s between the build queue list, the unit list and
	 * the building list.
	 */
	...
	private void updateUnitList() {
		...
		loop: for (UnitType unitType : spec.getBuildableUnitTypes()) {
			...
			if (!this.colony.hasAbility(Ability.BUILD, unitType, turn) && !this.featureContainer.hasAbility(Ability.BUILD, unitType, null)) {
	...
	private void updateBuildingList() {
		...
		loop: for (BuildingType buildingType : spec.getBuildingTypeList()) {
			...
			if (current.contains(buildingType) || hasBuildingType(buildingType) || unbuildableTypes.contains(buildingType) || !buildingType.needsGoodsToBuild()) {
	...
	private int getMaximumIndex(BuildableType buildableType) {
		...
		if (buildableType instanceof BuildingType) {
			...
			for (int index = 0; index < buildQueue.getSize(); index++) {
				...
				if ((foundUpgradesFrom && upgradesTo != null && upgradesTo.equals(toBuild)) || buildableType.hasAbility(Ability.BUILD, toBuild)) {
					return index;
				}
			}
			return buildQueueLastPos;
