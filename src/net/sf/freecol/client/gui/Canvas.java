...
public final class Canvas extends JDesktopPane {
	...
	private enum PopupPosition {
		ORIGIN, CENTERED, CENTERED_LEFT, CENTERED_RIGHT,
	}
	 ...
	 * @param toolBox
	 *            Should be set to true if the resulting frame is used as a toolbox
	 *            (that is: it should not be counted as a frame).
	 * @param popupPosition
	...
	private Point chooseLocation(Component comp, int width, int height, PopupPosition popupPosition) {
		Point p = null;
		if (comp instanceof FreeColPanel && (p = getSavedPosition(comp)) != null && (p.getX() < 0 || p.getX() >= getWidth() - width || p.getY() < 0 || p.getY() >= getHeight() - height)) {
			p = null;
		}
		int x = 0, y = 0;
		if (p != null) {
		...
		if (p != null && p.x >= 0 && p.x < getWidth() && p.y >= 0
				&& p.y < getHeight()) {
			x = p.x;
			y = p.y;
		}
	 ...
	 * @param h
	 *            The component height to use.
	 * @return A <code>Point</code> to place the component at or null on failure.
	 */
	 ...
	 *
	 * This is distinct from {@link #getExistingFreeColPanel} because there can be
	 * multiple ColonyPanels.
	 *
	 * @param colony
	 ...
	 * @return The given component if this is an internal frame or the first parent
	 *         that is an internal frame. Returns <code>null</code> if no internal
	 *         frame is found.
	 */
	 ...
	 * @param comp
	 *            The <code>Component</code> to use.
	 * @return A <code>Dimension</code> for the component or null if no saved size
	 *         is found.
	 */
	...
	/**
	 * Save an <code>int</code> value to the saved ClientOptions, using the name of
	 * the components class plus the given key as and identifier.
	 *
	 * @param className
	 ...
	 *            <code>FreeColPanel</code>, panel to show
	 * @param popupPosition
	 *            <code>PopupPosition</code> The generalized position to place the
	 *            panel.
	 * @param resizable
	 *            Should the panel be resizable?
	 */
	 ...
	 *
	 * @return A <code>Component</code> the <code>Canvas</code> is displaying, or
	 *         null if none found.
	 */
	 ...
	 * <p>
	 * Note that the previous implementation could throw exceptions in some cases,
	 * thus the change.
	 *
	...
	/**
	 * Set preferred size to saved size, or to the given <code>Dimension</code> if
	 * no saved size was found. Call this method in the constructor of a
	 ...
	 * @param choices
	 *            The <code>List</code> containing the ChoiceItems to create buttons
	 *            for.
	 * @return The corresponding member of the values array to the selected option.
	 */
	...
	/**
	 * Displays the colony panel of the given <code>Colony</code>. Defends against
	 * duplicates as this can duplicate messages generated by multiple property
	 * change listeners registered against the same colony.
	 *
	 * @param colony
	...
	/**
	 * Shows the panel that allows the user to choose which unit will emigrate from
	 * Europe.
	 *
	 * @param player
	...
	/**
	 * Display the first contact dialog (which is really just a non-modal confirm
	 * dialog).
	 *
	 * @param player
	 ...
	 *            The number of settlements the other player has (from the server,
	 *            other.getNumberOfSettlements() is wrong here!).
	 * @param handler
	 *            A <code>DialogHandler</code> for the dialog response.
	 */
	...
	/**
	 * Make image icon from an image. Use only if you know having null is possible!
	 *
	 * @param image
	...
	/**
	 * Displays a dialog for setting options when loading a savegame. The settings
	 * can be retrieved directly from {@link LoadingSavegameDialog} after calling
	 * this method.
	 *
	 * @param publicServer
	 *            Default value.
	 ...
	 * @param comment
	 *            An optional <code>StringTemplate</code> containing a commentary
	 *            message.
	 ...
	 *
	 * @param question
	 *            a <code>stringtemplate</code> describing the amount of tribute to
	 *            demand.
	 * @param maximum
	...
	/**
	 * Displays the colony panel of the given <code>Colony</code> following a spying
	 * action.
	 *
	 * @param tile
	...
	/**
	 * Shows a status message that cannot be dismissed. The panel will be removed
	 * when another component is added to this <code>Canvas</code>. This includes
	 ...
	 * @param x
	 *            The x-coordinate on the screen where the popup needs to be placed.
	 * @param y
	 *            The y-coordinate on the screen where the popup needs to be placed.
	 * @see TilePopup
	 */
