...
public class FontLibrary {
	...
	/**
	 * Create a <code>FontLibrary</code> without scaling. Probably not worth using,
	 * as you could just use the static methods.
	 */
	public FontLibrary() {
		this.scaleFactor = 1f;
	}
	 ...
	 * 
	 * @param scaleFactor
	 *            How much scaling should be applied. Typically the same value as in
	 *            ImageLibrary.
	 */
	 ...
	 * 
	 * @param fontName
	 *            Can be used to choose a different font from a user-provided name.
	 * @param scaleFactor
	...
	/**
	 * Create a <code>Font</code> in rare case one is needed without scaling. Do not
	 * use this for normal text, as it leaves out the scaling factor you should get
	 * from the appropriate ImageLibrary (there are 3 in use)! Exceptions are
	 * currently big headers and things where gui elements are not made flexible
	 * enough already to allow a changed size.
	 * 
	 * @param fontType
	 *            How the font should look like.
	 * @param fontSize
	 ...
	 * explicitly in the parameter. The equivalent of regular text, which would only
	 * complicate the source code and slow down the game if used, would be:
	...
	/**
	 * Create a scaled <code>Font</code> which can display all characters inside the
	 * given text string. This is mostly necessary for the header font. Thats
	 * because the currently used ShadowedBlack is missing support for CJK and
	 * others. Even some special glyphs for European languages like the triple-dot
	 * are missing.
	 * 
	 * @param string
