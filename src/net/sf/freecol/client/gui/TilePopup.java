...
public final class TilePopup extends JPopupMenu {
	 ...
	 *            The Canvas.
	 * @param tile
	 *            The <code>Tile</code> to create a popup for. The popup menu also
	 *            appears near this <code>Tile</code>.
	 */
	 ...
	 * @param indent
	 *            Should be <code>true</code> if the text should be indented on the
	 *            menu.
	...
	private void addTile(final Tile tile) {
		...
		add(menuItem);
		/*
		 * Don't set hasAnItem to true, we want the tile panel to open automatically
		 * whenever there is no other item on the list.
		 */
		// hasAnItem = true;
	}

	/**
	 * Returns true if this popup has at least one menuitem so that we know that we
	 * can show it to the user. Returns false if there are no menuitems.
	 *
