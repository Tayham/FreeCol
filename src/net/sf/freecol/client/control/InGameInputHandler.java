...
public final class InGameInputHandler extends InputHandler {
	...
	private Element animateAttack(Element element) {
		...
		if ((str = element.getAttribute("attackerTile")).isEmpty()) {
			throw new IllegalStateException(
					ATTACK_ANIMATION_FOR + player.getId() + " missing attacker tile attribute.");
		}
		final Tile attackerTile = game.getFreeColGameObject(str, Tile.class);
		if (attackerTile == null) {
			throw new IllegalStateException(ATTACK_ANIMATION_FOR + player.getId() + " omitted attacker tile: " + str);
		}

		if ((str = element.getAttribute("defenderTile")).isEmpty()) {
			throw new IllegalStateException(
					ATTACK_ANIMATION_FOR + player.getId() + " missing defender tile attribute.");
		}
		final Tile defenderTile = game.getFreeColGameObject(str, Tile.class);
		if (defenderTile == null) {
			throw new IllegalStateException(ATTACK_ANIMATION_FOR + player.getId() + " omitted defender tile: " + str);
		}
	...
	private Element animateMove(Element element) {
		...
		if (unitId.isEmpty()) {
			logger.warning(ANIMATION_FOR + player.getId() + " missing unitId.");
			return null;
		}
		...
		if (u == null) {
			logger.warning(ANIMATION_FOR + player.getId() + " missing unit:" + unitId);
			return null;
		}
		final Unit unit = u;
		...
		if (oldTileId.isEmpty()) {
			logger.warning(ANIMATION_FOR + player.getId() + " missing oldTileId");
			return null;
		}
		final Tile oldTile = game.getFreeColGameObject(oldTileId, Tile.class);
		if (oldTile == null) {
			logger.warning(ANIMATION_FOR + player.getId() + " missing oldTile: " + oldTileId);
			return null;
		}

		String newTileId = element.getAttribute("newTile");
		if (newTileId.isEmpty()) {
			logger.warning(ANIMATION_FOR + player.getId() + " missing newTileId");
			return null;
		}
		final Tile newTile = game.getFreeColGameObject(newTileId, Tile.class);
		if (newTile == null) {
			logger.warning(ANIMATION_FOR + player.getId() + " missing newTile: " + newTileId);
			return null;
		}
