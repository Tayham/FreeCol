...
final class ReceivingThread extends Thread {
	 ...
	 * 
	 * This is just a buffered input stream that signals end-of-stream when a given
	 * token {@link #END_OF_STREAM} is encountered. In order to continue receiving
	 * data, the method {@link #enable} has to be called. Calls to
	 * <code>close()</code> have no effect, the underlying input stream has to be
	 * closed directly.
	 */
	private static class FreeColNetworkInputStream extends InputStream {
		 ...
		 * 
		 * @param in
		 *            The input stream in which this object should get the data from.
		 */
		...
		/**
		 * Prepares the input stream for a new message. Makes the subsequent calls to
		 * <code>read</code> return the data instead of <code>-1</code>.
		 */
		public void enable() {
			this.wait = false;
		}
		 ...
		 * @return The actual number of bytes read, or -1 if the message has ended
		 *         ({@link #END_OF_STREAM} was encountered).
		 */
		@Override
	...
	/**
	 * Gets the next network reply identifier that will be used when identifing a
	 * network message.
	 * 
	 ...
	 *            The identifier of the message the calling thread should wait for.
	...
	/**
	 * Receives messages from the network in a loop. This method is invoked when the
	 * thread starts and the thread will stop when this method returns.
	 */
	@Override
	public void run() {
		int timesFailed = 0;
