...
public abstract class ServerAPI {
	...
	/**
	 * Loop sending requests and handling replies from the server until they reduce
	 * to null.
	 *
	 * @param request
	...
	/**
	 * Sends the specified message to the server and returns the reply, if it has
	 * the specified tag. Handle "error" replies if they have a messageID or when in
	 * debug mode. This routine allows code simplification in much of the following
	 * client-server communication.
	 *
	 * In following routines we follow the convention that server I/O is confined to
	 * the ask<foo>() routine, which typically returns true if the server
	 * interaction succeeded, which does *not* necessarily imply that the actual
	 * substance of the request was allowed (e.g. a move may result in the death of
	 * a unit rather than actually moving).
	 *
	 * @param message
	 ...
	 * @param claimant
	 *            The <code>Unit</code> or <code>Settlement</code> that is claiming
	 *            the tile.
	 * @param price
	 *            The amount to pay.
	 * @return True if the server interaction succeeded.
	...
	/**
	 * Server query-response for the special case of deciding to explore a rumour
	 * but then declining not to investigate the strange mounds.
	 *
	 * @param unit
	 ...
	 *
	 * @param slot
	 *            The slot from which the unit migrates, 1-3 selects a specific one,
	 *            otherwise the server will choose one.
	 ...
	 *         failure to incite and >0 for successful incitement, negative if the
	 ...
	 * @param goods
	 *            A list of <code>Goods</code>, if empty this is a query as to what
	 *            is to be looted which is filled into the list, if non-empty, then
	 *            the list of goods to loot.
	 * @return True if the server interaction succeeded.
	 */
	 ...
	 *
	 * @param mapOptions
	 *            The <code>OptionGroup</code> containing the map generator options.
	 * @return True if the server interaction succeeded.
	 */
