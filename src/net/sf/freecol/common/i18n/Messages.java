...
public class Messages {
	 ...
	 * command line processing, which in turn requires at least the basic localized
	 * message resources to be loaded. So this routine is called separately when the
	 * mods are finally loaded.
	 *
	 * @param locale
	...
	/**
	 * Returns the preferred key if it is contained in the message bundle and the
	 * default key otherwise. This should be used to select the most specific
	 ...
	 *
	 * Favour the .name form, but degrade gracefully if it is not present. If .name
	 * is present, also look for a description.
	 *
	 * @param named
	...
	public static StringTemplate getUnitLabel(String name, String typeId, int number, String nationId, String roleId,
		...
		if (name == null) {
			...
			// %name% (%nation% %role%/%type%/%extra%) | "Bob (Dutch Soldier/Free
			// Colonist/50 muskets)"
			ret = StringTemplate.label("").addName(name).addName(" (").add(nameKey(nationId)).addName(" ")
					.add(nameKey(roleKey)).addName("/").addStringTemplate(type).addName(")");
		}
	 ...
	 * message bundle. Returns the key as the value if there is no mapping found!
	 *
	 * @param messageId
	...
	/**
	 * Replace all choice formats in the given string, using keys and replacement
	 * values from the given template, which may be null.
	 *
	 ...
	 * character, followed by one or several choices separated by pipe characters.
	 * If there is only one choice, it must be a message identifier or a variable.
	 * Otherwise, each choice consists of a key and a value separated by an
	 * assignment character. Example: "{{tag:selector|key1=val1|key2=val2}}".
	 *
	 * @param input
	 *            a <code>String</code> value
	 * @param template
