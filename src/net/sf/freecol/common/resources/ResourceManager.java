...
public class ResourceManager {
	...
	/**
	 * The following fields are mappings from resource IDs to resources. A mapping
	 * is defined within a specific context. See the comment on each field's setter
	...
	/**
	 * All the mappings above merged into this single ResourceMapping according to
	 * precendence.
	 */
	...
	/**
	 * Sets the mappings specified for a Total Conversion (TC). Do not access the
	 * mapping after the call.
	 *
	 * @param mapping
	 *            The mapping between IDs and files.
	...
	/**
	 * Sets the mappings specified in a scenario. Do not access the mapping after
	 * the call.
	 *
	 * @param mapping
	 *            The mapping between IDs and files.
	...
	/**
	 * When it is anticipated that a resource could not exist use this method for
	 * checking beforehand. Other metods below are specializations running faster or
	...
	/**
	 * Returns a list of all keys starting with the given prefix and ending with the
	 * given suffix.
	 *
	 * @param prefix
	 *            the prefix
	 ...
	 * @param key
	 *            The resource to get.
	 * @return The resource if there is one with the given resource key and type, or
	 *         else <code>null</code>.
	 */
	 ...
	 * images from any of the below methods is an error. To make modding easier and
	 * prevent edited resource files from crashing the game, a replacement image is
	...
	/**
	 * Returns the image specified by the given name. Please, avoid using too many
	 * different scaling factors! For each is a scaled image cached here for a long
	 * time, which wastes memory if you are not careful.
	 *
	 * @param key
	 *            The name of the resource to return.
	 * @param scale
	 *            The size of the requested image (with 1 being normal size, 2 twice
	 *            the size, 0.5 half the size etc). Rescaling will be performed
	 *            unless using 1.
	 * @return The image identified by <code>resource</code>.
	 */
	...
	/**
	 * Returns the image specified by the given name. Please, avoid using this, as
	 * for each size another scaled version of the image is cached!
	 *
	 * @param key
	 *            The name of the resource to return.
	 * @param size
	 *            The size of the requested image. Rescaling will be performed if
	 *            necessary.
	 * @return The image identified by <code>resource</code>.
	 */
	...
	/**
	 * Returns the animation specified by the given name. As the artwork is still
	 * incomplete and animations exist only for some military units, null can still
	 * be returned in many cases. FIXME: Check using hasResource before calling
	 * this, then replace null return with calling FreeColClient.fatal on error.
	 *
	 * @param key
	 *            The name of the resource to return.
	 * @return The animation identified by <code>resource</code> or
	 *         <code>null</code> if there is no animation identified by that name.
	 */
	 ...
	 *            The name of the resource to return.
	 * @param scale
	 *            The size of the requested animation (with 1 being normal size, 2
	 ...
	 * non-existing color is an error and will return magenta as replacement color
	 * to prevent crashes when modding.
	 *
	 * @param key
	 *            The name of the resource to return.
	 ...
	 * error and will result in only getting the emergency font to prevent crashes
	 * when modding.
	 *
	 * @param key
	 *            The name of the resource to query.
	 * @return The <code>Font</code> found in a FontResource, which may default to
	 *         the Java default font if the resource failed to load.
	 */
	...
	/**
	 * Gets an audio resource with the given name. This can return null as there as
	 * not all necessary sounds have been added to the game. FIXME: Change calling
	 * code to check using hasResource, then replace null return with calling
	 ...
	 * return null as there is only one video in FreeCol. FIXME: Consider calling
	 * FreeColClient.fatal on error.
	 * 
	 * @param key
	 *            The name of the resource to return.
	...
	/**
	 * Gets a FAFile resource with the given name. This can return null as there is
	 * only one FAFile in FreeCol. FIXME: Consider calling FreeColClient.fatal on
	 * error.
	 *
	 * @param key
	 *            The name of the resource to query.
