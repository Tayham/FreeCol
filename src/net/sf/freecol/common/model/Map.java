...
public class Map extends FreeColGameObject implements Location {
	...
	/**
	 * Possible actions by the unit travelling along a path in consideration of the
	 * next tile.
	 */
	private enum MoveStep {
		FAIL, BYLAND, BYWATER, EMBARK, DISEMBARK
	}
	...
	/**
	 * The layers included in the map. The RIVERS layer includes all natural tile
	 * improvements that are not resources. The NATIVES layer includes Lost City
	 * Rumours as well as settlements.
	 */
	public enum Layer {
	...
	public static final class Position {
		...
		/**
		 * Creates a new <code>Position</code> object with the given coordinates.
		 *
		 * @param posX
		...
		/**
		 * Creates a new <code>Position</code> object with the coordinates of a supplied
		 * tile.
		 *
		 * @param tile
		...
		/**
		 * Creates a new <code>Position</code> from an existing one with an optional
		...
		/**
		 * Gets the distance in tiles between two map positions. With an isometric map
		 * this is a non-trivial task. The formula below has been developed largely
		 * through trial and error. It should cover all cases, but I wouldn't bet my
		 * life on it.
		 *
		 * @param ax
	...
	/**
	 * Create a new <code>Map</code> from an <code>Element</code> in a DOM-parsed
	 * XML-tree.
	 *
	 * @param game
	 *            The enclosing <code>Game</code>.
	...
	/**
	 * Creates a new <code>Map</code> with the given object identifier. The object
	...
	/**
	 * Gets the Tile at position (x, y). 'x' specifies a column and 'y' specifies a
	 * row. (0, 0) is the Tile at the top-left corner of the Map.
	 *
	 * @param x
	 *            The x-coordinate of the <code>Tile</code>.
	 * @param y
	 *            The y-coordinate of the <code>Tile</code>.
	 * @return The <code>Tile</code> at (x, y), or null if the position is invalid.
	 */
	 ...
	 * @param name
	 *            The region name.
	 * @return The <code>Region</code> with the given name, or null if not found.
	 */
	...
	/**
	 * Gets the direction a unit needs to move in order to get from <code>t1</code>
	 * to <code>t2</code>
	 *
	 * @param t1
	 *            The tile to move from.
	 * @param t2
	 *            The target tile if moving from <code>t1</code> in the direction
	 *            returned by this method.
	 * @return The direction you need to move from <code>t1</code> in order to reach
	 *         <code>t2</code>, or null if the two specified tiles are not
	 *         neighbours.
	 */
	 ...
	 * @return The approximate direction from source to direction, or null if source
	 ...
	 * @return The adjacent <code>Tile</code> in the specified direction, or null if
	 *         invalid.
	 */
	...
	/**
	 * Unified argument tests for full path searches, which then finds the actual
	 * starting location for the path. Deals with special cases like starting on a
	 * carrier and/or high seas.
	 *
	 * @param unit
	...
	/**
	 * Destination argument test for path searches. Find the actual destination of a
	 * path.
	 *
	 * @param end
	 ...
	 * @return A path to the best entry location tile to arrive on the map at, or
	 *         null if none found.
	 */
	...
	/**
	 * Find the quickest path for a unit (with optional carrier) from a start tile
	 * to an end tile.
	 *
	 * @param unit
	 *            The <code>Unit</code> to find the path for.
	 ...
	 * @return A path starting at the start tile and ending at the end tile, or null
	 *         if none found.
	 */
	private PathNode findMapPath(Unit unit, Tile start, Tile end, Unit carrier, CostDecider costDecider,
		...
		} else if (offMapUnit != null || (unit != null && unit.isOnCarrier() && !start.isLand() && end.isLand()
	 ...
	 * @param lb
	 *            An optional <code>LogBuilder</code> to log to.
	 * @return A path starting at the start location and ending at the end location,
	...
	public PathNode findPath(final Unit unit, final Location start, final Location end, final Unit carrier,
		...
		if (realEnd instanceof Tile && !((Tile) realEnd).isExplored()) {
			...
			if (offMapUnit == null || !offMapUnit.getType().canMoveToHighSeas() || ((p = getBestEntryPath(unit, (Tile) realEnd, carrier, costDecider)) == null)) {
				path = null;
			...
			if (offMapUnit == null || !offMapUnit.getType().canMoveToHighSeas() || ((p = searchMap(unit, (Tile) realStart, GoalDeciders.getHighSeasGoalDecider(), costDecider,
	...
	/**
	 * Searches for a goal. Assumes units in Europe return to their current entry
	 * location, which is not optimal most of the time. Returns the full path
	 ...
	 * @throws IllegalArgumentException
	 *             If the unit is null, or the start location does not make sense,
	...
	public PathNode search(final Unit unit, Location start, final GoalDecider goalDecider,
		...
		if (realStart instanceof Europe) {
			// Fail fast if Europe is unattainable.
			if (offMapUnit == null || !offMapUnit.getType().canMoveToHighSeas() || ((p = searchMap(unit, (Tile) offMapUnit.getEntryLocation(), goalDecider, costDecider, maxTurns,
...
// FIXME: do something better.
	 ...
	 *
	 * Beware! This is special case code for partially constructed paths that do not
	 * yet have valid .next links, so we can not use the generic PathNode routines.
	 *
	 * @param path
	 *            The path the search.
	...
	private class MoveCandidate {
		...
		/**
		 * Creates a new move candidate where a cost decider will be used to work out
		 * the new moves and turns left.
		 *
		 * @param unit
	 ...
	 *
	 * Using A* with a List (closedMap) for marking the visited nodes and using a
	 * PriorityQueue (openMapQueue) for getting the next edge with the least cost.
	 * This implementation could be improved by having the visited attribute stored
	 * on each Tile in order to avoid both of the HashMaps currently being used to
	 * serve this purpose.
	 *
	 * If the SearchHeuristic is not supplied, then the algorithm degrades
	 * gracefully to Dijkstra's algorithm.
	 *
	 * The data structure for the open list is a combined structure: using a HashMap
	 * for membership tests and a PriorityQueue for getting the node with the
	 * minimal f (cost+heuristics). This gives O(1) on membership test and O(log N)
	 * for remove-best and insertions.
	 *
	 * @param unit
	 *            The <code>Unit</code> to find a path for.
	 * @param start
	 ...
	 * @param lb
	 *            An optional <code>LogBuilder</code> to log to.
	 * @return A path to a goal determined by the given <code>GoalDecider</code>.
	 */
	private PathNode searchMap(final Unit unit, final Tile start, final GoalDecider goalDecider,
		...
		Unit currentUnit = start.isLand()
				? ((start.hasSettlement() && start.getSettlement().isConnectedPort() && unit != null
	 ...
	 *            The starting <code>Tile</code>.
	 * @param goalDecider
	 *            A <code>GoalDecider</code> that chooses the goal, which must be
	 *            capable of tolerating a null unit.
	 * @param radius
	 ...
	 * The center tile is never included in the returned tiles, and all returned
	 * tiles are valid.
	 */
	private final class CircleIterator implements Iterator<Tile> {
		 ...
		 *
		 * @return The distance from the center tile this <code>CircleIterator</code>
	 ...
	 * @return The first land tile found within the radius, or null if none found.
	 */
	 ...
	 * calling methods doing redundant checks, which would slow down map display.
	 * 
	 * @param x
	 ...
	 *            The distance between the land and the high seas (given in tiles).
	 * @param maxDistanceToEdge
	 *            The maximum distance a high sea tile can have from the edge of the
	 *            map.
	 */
	...
	/**
	 * Sets the high seas count for all tiles connected to the high seas. Any ocean
	 * tiles on the map vertical edges that do not have an explicit false
	 * moveToEurope attribute are given a true one.
	 *
	 * Set all high seas counts negative, then start with a count of zero for tiles
	 * with the moveToEurope attribute or of a type with that ability. Iterate
	 * outward by neighbouring tile, incrementing the count on each pass, stopping
	 * at land. Thus, only the coastal land tiles will have a non-negative high seas
	 * count. This significantly speeds up the colony site evaluator, as it does not
	 * have to try to find a path to Europe for each tile.
	 */
