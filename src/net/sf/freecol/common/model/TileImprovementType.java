...
public final class TileImprovementType extends FreeColGameObjectType {
	...
	/**
	 * The layer a TileItem belongs to. Items with higher zIndex will be displayed
	 * above items with a lower zIndex. E.g. the LostCityRumour would be displayed
	 * above the Plow improvement.
	 */
	private int zIndex;

	/**
	 * Can this improvement expose a resource when completed? This should only apply
	 * to improvement types that change the underlying tile type (e.g. clearing
	 * forests).
	 */
	 ...
	 * the tile. This only applies to TileImprovementTypes that change the
	 ...
	 *
	 * Only applies if movementCost is positive (see spec). Do not return zero from
	 * a movement costing routine or units get free moves!
	 *
	 * @param originalCost
