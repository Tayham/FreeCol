...
public final class Tile extends UnitLocation implements Named, Ownable {
	...
	/**
	 * Information that is internal to the native settlements, and only updated on
	 * close contact.
	 */
	...
	/**
	 * Flag to assign to the high seas count to flag that the high seas connectivity
	 * needs recalculation after reading in the map.
	 */
	...
	/**
	 * The maximum distance that will still be considered "near" when determining
	 * the location name.
	 *
	...
	/**
	 * The type of the tile. Beware: this may appear to be null in the client when
	...
	/**
	 * Indicates which settlement owns this tile (null indicates no owner). A colony
	 * owns the tile it is located on, and every tile it has claimed by successfully
	 * moving a worker on to it. Note that while units and settlements are owned by
	 * a player, a tile is owned by a settlement.
	 */
	 ...
	 * map. That is, all land tiles with the same contiguity number can be reached
	 * by a land unit on any of those tiles in the absence of extra-geographic
	 * blockages like settlements and other units. Similarly for water tiles/naval
	 * units.
	 *
	 * This is used to quickly scope out the sort of paths available to a unit
	 * attempting to reach some destination. It only needs serialization from server
	 * to client, as it is set by the TerrainGenerator on map import or creation.
	 */
	...
	/**
	 * Create a new <code>Tile</code> with the given identifier. The object should
	...
	/**
	 * Put a settlement onto this tile. A tile can only have one settlement located
	 * on it. The settlement will also become the owner of this tile.
	 *
	 * -til: Changes appearance.
	 *
	 * @param settlement
	...
	/**
	 * Quick test whether this tile is trivially blocked to moves from a unit. This
	 * is a simplification, use getMoveType().isProgress() for the full details.
	 *
	 * @param unit
	 ...
	 * @return The <code>IndianSettlementInternals</code> for the given player, or
	 ...
	 * @param type
	 *            The <code>TileImprovementType</code> to look for.
	 * @return The <code>TileImprovement</code> of the requested type found, or null
	 *         if none.
	 */
	 ...
	 *
	 * @return True if this <code>Tile</code> has a <code>LostCityRumour</code> on
	 *         it.
	 */
	 ...
	 *
	 * @return The <code>LostCityRumour</code> on this <code>Tile</code>, or null if
	 *         none found.
	 */
	 ...
	 * <code>TileImprovementType</code>. It will check if it is valid for this
	 * <code>TileType</code>.
	 *
	 * @param workType
	 *            The <code>TileImprovementType</code> to check.
	 * @return The number of turns it should take a non-expert pioneer to finish the
	 *         work.
	 */
	 ...
	 * list comprises all natural disasters that can strike a tile of this type or a
	 * completed tile improvement present.
	 *
	 ...
	 * any improvements on it (road/plow/etc) from <code>TileItemContainer</code>.
	 *
	 ...
	 * @return A suitable <code>StringTemplate</code>, or null if this tile is not
	 *         close enough to the colony to be a colony tile.
	 */
	 ...
	 * @return The direction to the other <code>Tile</code>, or null if the other
	 *         tile is not a neighbour.
	 */
	 ...
	 * lake. If this is true for a land tile with a colony, the colony can build
	 * docks.
	 *
	...
	/**
	 * Gets all the tiles surrounding a tile within the given range. The center tile
	 * is not included.
	 *
	 * @param range
	 ...
	 * lower bounds.
	 *
	 * getSurroundingTiles(r) is equivalent to getSurroundingTiles(1, r), thus this
	 * tile is included if rangeMin is zero.
	 *
	 * @param rangeMin
	 ...
	 *
	 * @return True if at least one neighbouring <code>Tile</code>s is unexplored.
	 */
	...
	/**
	 * Finds a safe tile to put a unit on, near to this one. Useful on return from
	 * Europe.
	 *
	 * @param player
	 ...
	 * Useful when planning an attack.
	 *
	 * @param player
	...
	/**
	 * Get the adjacent land tile with the best defence bonus. Useful for incoming
	 * attackers as a disembark site.
	 *
	 * @param player
	 *            A <code>Player</code> to use to check for tile access.
	 ...
	 * to this tile. To be safe, the tile must be adjacent to this one but not
	 * adjacent to a dangerous settlement.
	 *
	 * @param unit
	 ...
	 * setType(), whereas this routine should be called for the special case of a
	 * change of an existing tile type (e.g. pioneer clearing forest).
	 *
	 * -til: Changes appearance.
	 *
	 * @param type
	 ...
	 *
	 * @param settlement
	 *            The new owning <code>Settlement</code> for this <code>Tile</code>.
	 */
	...
	/**
	 * Change the tile ownership. Also change the owning settlement as the two are
	 * commonly related.
	 *
	 * -til: Changes appearance.
	 *
	...
	/**
	 * A colony is proposed to be built on this tile. Collect warnings if this has
	 * disadvantages.
	 *
	 * @param unit
	 ...
	 *            An optional <code>ProductionType</code> to use, if null the best
	...
	/**
	 * Gets the maximum potential for producing the given type of goods. The maximum
	 * potential is the potential of a tile after the tile has been plowed/built
	 * road on.
	 *
	 * @param goodsType
	...
	public List<AbstractGoods> getSortedPotential(UnitType unitType, Player owner) {
		...
		if (getType() != null) {
			...
			Collections.sort(goodsTypeList,
					(owner == null || owner.getMarket() == null) ? AbstractGoods.abstractGoodsComparator
	 ...
	 * it. Useful when the cache needs to be cleared forcibly such as when a native
	 ...
	 *
	 * @param player
	 *            A <code>Player</code> that currently may not be able to see the
	 *            tile, but will as a result of the change, and so should not cache
	 *            it.
	 */
	...
	/**
	 * A change may have occured on this tile. Establish caches where needed. Use
	 * the copied tile if supplied (which should have been created previously with
	 * {@link #getTileToCache},
	 *
	 * @param copied
	...
	/**
	 * Updates the information about the native settlement on this <code>Tile</code>
	...
	/**
	 * Set native settlement information. Do not check the current map state as we
	 * might leak destruction information.
	 *
	 * @param player
	 ...
	 * whether exploration has happened. In the client there are no cached tiles,
	 * but if the tile is explored the server will have updated the client with the
	 * tile type (checked by isExplored()).
	 *
	 * @param player
	 *            The <code>Player</code>.
	 ...
	 *
	 * @param direction
	 *            The <code>Direction</code> from this tile to the nearby location.
	 * @param location
	 *            A <code>StringTemplate</code> describing the location.
	 * @return A <code>StringTemplate</code> stating that the location is nearby.
	 */
