...
public abstract class UnitLocation extends FreeColGameObject implements Location {
	...
	public enum NoAddReason {
		...
		CAPACITY_EXCEEDED,
		/**
		 * Locatable can not be added because the Location is occupied by objects
		 * belonging to another player.
		 */
		OCCUPIED_BY_ENEMY,
		/**
		 * Locatable can not be added because the Location belongs to another player and
		 * does not admit foreign objects.
		 */
		OWNED_BY_ENEMY,
	 ...
	 * carrier.
	 *
	 * @return The total number of <code>Unit</code>s at this <code>Location</code>.
	 */
	...
	/**
	 * Checks if there is a useable carrier unit with a specified minimum amount of
	...
	/**
	 * Gets the reason why a given <code>Locatable</code> can not be added to this
	 * Location.
	 *
	 * Be careful to test for unit presence last before success (NoAddReason.NONE)
	 * except perhaps for the capacity test, so that we can treat ALREADY_PRESENT as
	 * success in some cases (e.g. if the unit changes type --- does it still have a
	 * required skill?)
	 *
	...
	/**
	 * Gets the maximum number of <code>Units</code> this Location can hold. To be
	 ...
	 * @return Negative if the goods are unavailable, otherwise the price (may be
	 *         zero).
	 */
	...
	protected void readChild(FreeColXMLReader xr) throws XMLStreamException {
		...
		if (Unit.getXMLElementTagName().equals(tag)) {
			...
			if (u.getLocation() != this) {
				logger.warning("Fixing bogus unit location for " + u.getId() + ", expected " + getId()
