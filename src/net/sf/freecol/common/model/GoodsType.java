...
public final class GoodsType extends FreeColGameObjectType {
	...
	/**
	 * The number of units required to breed this type of goods. This obviously only
	 * applies to animals.
	 */
	private int breedingNumber = INFINITY;

	/**
	 * The price of this type of goods. This is only used for goods that can not be
	 * traded in the market, such as hammers.
	 */
	 ...
	 * actual initial sales price in a particular Market may be higher. This method
	 * is only used for initializing Markets.
	 *
	 ...
	 * that can not be produced from any other. The last element of the production
	 * chain is the goods type itself.
	 *
	 * Note that this does not really handle goods that are stored as something else
	 * as is the case for grain etc. FIXME: fix or retire this routine?
	 *
	 ...
	 * type.
	 *
	 * @return A set of equivalent <code>GoodsType</code>s, which must include this
	 *         one.
	 */
	 ...
	 *
	 * The "derived" fields are: buildingMaterial + makes - buildingMaterial depends
	 * on whether a GoodsType is present on a BuildableType requiredGoods list -
	 * makes depends on whether a GoodsType madeFrom field refers to another
	 * 
	 * This is called from Specification.clean() when the specification is fully
	 * read. We must wait until then as the made-from field can change in extended
	 * specifications and mods. The current example of which is horses, which is
	 * made-from food in the classic ruleset and made-from grain in the freecol
	 * ruleset.
	 *
	 * @param spec
