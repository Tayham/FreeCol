...
public class IndianSettlement extends Settlement implements TradeLocation {
	...
	/**
	 * This is the skill that can be learned by Europeans at this settlement. At the
	 * server side its value will be null when the skill has already been taught to
	 * a European. At the client side the value null is also possible in case the
	 * player hasn't checked out the settlement yet.
	 */
	 ...
	 *
	 * At the client side, only the information regarding the player on that client
	 * should be included.
	 */
	...
	/**
	 * Stores the alarm levels. <b>Only used by AI.</b> "Alarm" means: Tension with
	 * respect to a player from an IndianSettlement. Alarm is overloaded with the
	 * concept of "contact". If a settlement has never been contacted by a player,
	 * alarm.get(player) will be null. Acts causing contact initialize this
	 * variable.
	 */
	...
	/**
	 * Creates a new <code>IndianSettlement</code> with the given identifier. The
	 ...
	 * <code>IndianSettlement</code>. Returns true if the Unit was removed.
	 *
	 * @param unit
	 ...
	 *
	 * -vis: This routine has visibility implications when enhanced missionaries are
	 * enabled. -til: This changes the tile appearance.
	 *
	 * @param missionary
	...
	public StringTemplate getMostHatedLabel(boolean contacted) {
		return contacted
				? ((mostHated == null) ? StringTemplate.key("model.indianSettlement.mostHatedNone")
	 ...
	 * Initialize tension level to the general level with respect to the contacting
	 * player--- effectively the average reputation of this player with the overall
	 * tribe.
	 *
	 * @param player
	...
	/**
	 * Is this settlement worth scouting? That is, has it been contacted, but not
	 * scouted already, or visited when the "Chief contact" option is set.
	 *
	 * @param player
	 ...
	 * @return The current alarm level or null if the settlement has not encoutered
	 *         the player.
	 */
	 ...
	 *
	 * @param unit
	 *            The <code>Unit</code> that proposes to contact this settlement.
	...
	/**
	 * Gets the amount of gold this <code>IndianSettlment</code> is willing to pay
	 * for the given <code>Goods</code>.
	 *
	 ...
	 *
	 * It is only meaningful to call this method from the server, since the
	 * settlement's {@link GoodsContainer} is hidden from the clients. The AI uses
	 * it though so it stays here for now. Note that it takes no account of whether
	 * the native player actually has the gold.
	 *
	...
	/**
	 * Gets the amount of gold this <code>IndianSettlment</code> is willing to sell
	 * the given <code>Goods</code> for.
	 *
	 * It is only meaningful to call this method from the server, since the
	 * settlement's {@link GoodsContainer} is hidden from the clients.
	 *
	 ...
	 * @return The maximum amount, of the given type of goods, that can be produced
	 *         in one turn.
	 */
	...
	/**
	 * Add some initial goods to a newly generated settlement. After all, they have
	...
	protected void readChildren(FreeColXMLReader xr) throws XMLStreamException {
		...
		for (Unit u : getUnitList()) {
			if (u.getLocation() != this) {
				...
				logger.warning("Fixing unit location" + " from " + u.getLocation() + " to " + getId());
			}
		}
		// end @compat
	}
