...
public class Game extends FreeColGameObject {
	...
	static {
		...
		serverClasses.put(Game.class, net.sf.freecol.server.model.ServerGame.class);
	...
	/**
	 * References to all objects created in this game. Serialization is not needed
	 * directly as these must be completely within { players, unknownEnemy, map }
	 * which are directly serialized.
	 */
	 ...
	 * complex combat models will be implemented in future. As soon as that happens,
	 * we will also have to make the combat model selectable.
	 */
	...
	/**
	 * A FreeColGameObjectListener to watch the objects in the game. Usually this is
	 * the AIMain instance. FIXME: is this better done with a property change
	 * listener?
	 */
	 ...
	 *
	 * @param mgo
	 *            An <code>OptionGroup</code> containing the map generator options.
	 */
	 ...
	 * <code>FreeColGameObject</code>.
	 *
	 * This <code>Iterator</code> should be iterated once in a while since it cleans
	 * the <code>FreeColGameObject</code> cache. Very few routines call this any
	 * more, so there is a thresholded call in removeFreeColGameObject to ensure the
	 * cache is still cleaned. Reconsider this if the situation changes.
	 *
	...
	/**
	 * Gets the unique identifier for this game. A game UUID persists in save game
	 * files.
	 *
	 ...
	 * @return The <code>Player</code> of the given nation, or null if not found.
	 */
	 ...
	 *            returned.
	 * @return The <code>Player</code> after the <code>beforePlayer</code> in the
	 *         list which determines the order each player becomes the current
	 *         player.
	 * @see #getNextPlayer
	 */
	...
	/**
	 * Get the client player this thread is operating for. If in the server there
	 * will be none.
	 *
	 ...
	 * @return The <code>Settlement</code> found, or <code>null</code> if there is
	 *         no known <code>Settlement</code> with the specified name (the
	 ...
	 * @return The corresponding server class, or the original class if none found.
	 */
