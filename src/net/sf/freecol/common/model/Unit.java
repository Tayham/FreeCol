...
public class Unit extends GoodsLocation implements Consumer, Locatable, Movable, Nameable, Ownable {
	...
	/**
	 * A large number of turns, denoting pathfinding failure. Do not use INFINITY as
	 * further calculation might use this.
	 */
	...
	/**
	 * The number of turns until the work is finished (e.g. sailing, improving), or
	 * '-1' if a Unit can stay in its state forever.
	 */
	...
	/**
	 * The amount of goods carried by this unit. This variable is only used by the
	 * clients. A negative value signals that the variable is not in use.
	 *
	...
	/**
	 * Creates a new <code>Unit</code> with the given identifier. The object should
	 * later be initialized by calling {@link #readFromXML(FreeColXMLReader)}.
	 *
	 * @param game
	 *            The enclosing <code>Game</code>.
	 * @param id
	 ...
	 * default - The specific name of the unit if it has one The NATIONAL template
	 * adds the nation The FULL template adds equipment annotations
	 *
	 * @param ult
	 ...
	 *
	 * @return A <code>StringTemplate</code> describing where this unit is going.
	 */
	...
	/**
	 * Checks if this is a trading <code>Unit</code>, meaning that it can trade with
	 * settlements.
	 *
	...
	/**
	 * Checks if this <code>Unit</code> is a `colonist'. A unit is a colonist if it
	 * is European and can build a new <code>Colony</code>.
	 *
	...
	/**
	 * Checks if this unit is a person, that is not a ship or wagon. Surprisingly
	 * difficult without explicit enumeration because model.ability.person only
	 * arrived in 0.10.1.
	 *
	 ...
	 *            FORTIFIED, ...}.
	 * @return True if the <code>Unit</code> state can be changed to the new value.
	 */
	 ...
	 * has left.
	 *
	 * If the work needs turns to be completed (for instance when plowing), then the
	 * moves the unit has still left will be used up. Some work (basically building
	 * a road with a hardy pioneer) might actually be finished already in this
	 * method-call, in which case the state is set back to UnitState.ACTIVE.
	 *
	 * @param s
	 *            The new state for this Unit. Should be one of {UnitState.ACTIVE,
	 *            UnitState.FORTIFIED, ...}.
	 ...
	 * Ideally it should be in ServerUnit but we keep it here for the benefit of the
	 * test suite.
	 *
	 * @param owner
	 ...
	 *
	 * @param roles
	 *            The list of <code>Role</code>s to filter, if null all available
	 *            roles are used.
	...
	/**
	 * Checks whether this <code>Unit</code> is at sea off the map, or on board of a
	 * carrier that is.
	 *
	 ...
	 *
	 * @param moves
	 *            The new amount of moves left this <code>Unit</code> should have.
	 */
	 ...
	 * Updates the work location production and the unit experience type if
	 * necessary.
	 *
	 * @param type
	 *            The <code>GoodsType</code> to produce.
	...
	/**
	 * Gets the experience of this <code>Unit</code> at its current experienceType.
	 *
	...
	/**
	 * Sets the experience of this <code>Unit</code> at its current experienceType.
	 *
	 * @param experience
	 ...
	 *
	 * @param imp
	 *            The new <code>TileImprovement</code> the pioneer is to work on.
	 */
	 ...
	 *            The number of turns of training this <code>Unit</code> has given.
	 * @see #getNeededTurnsOfTraining
	 */
	...
	/**
	 * Gets the number of turns this unit has to train to educate a student. This
	 * value is only meaningful for units that can be put in a school.
	 *
	 ...
	 * switches owners willingly. Currently only Converts do this, but it opens the
	 ...
	 *
	 * Ethnicity is inherited from the inhabitants of the place where the Unit was
	 * born. Allows former converts to become native-looking colonists.
	 *
	 ...
	 *
	 * This is currently only used for damaged ships, but might get an extended use
	 * later.
	 *
	...
	public List<TradeRouteStop> getCurrentStops() {
		...
		List<TradeRouteStop> stops = new ArrayList<>(getTradeRoute().getStops());
	 ...
	 *
	 * Paul Revere makes an unarmed colonist in a settlement pick up a stock-piled
	 * musket if attacked, so the bonus should be applied for unarmed colonists
	 * inside colonies where there are muskets available. Natives can also auto-arm.
	 *
	 * @return A <code>Role</code> that can be automatically assumed by this unit,
	 *         or null if none.
	 */
	 ...
	 * @return The <code>Role</code> available to this unit as a result of capturing
	 *         the loser equipment.
	 */
	 ...
	 * defend as is. To be specific an unarmed colonist is not defensive yet, even
	 * if Paul Revere and stockpiled muskets are available. That check is only
	 * performed on an actual attack.
	 *
	 * A settlement is lost when there are no more defensive units.
	 *
	 * @return True if this is a defensive unit meaning it can be used to defend a
	 *         <code>Colony</code>. This would normally mean that a defensive unit
	 *         also will be offensive.
	 */
	public boolean isDefensiveUnit() {
		return (unitType.isDefensive() || getRole().isDefensive()) && !isCarrier(); // Not wagons or ships
	}
	 ...
	 * there is no current defender, or if the alternate unit is better armed, or
	 * provides greater defensive power and does not replace a defensive unit
	...
	public static boolean betterDefender(Unit defender, double defenderPower, Unit other, double otherPower) {
		return defender == null || (defender.isPerson() && other.isPerson() && !defender.isArmed() && other.isArmed()) || ((!defender.isPerson() || !other.isPerson() || !defender.isArmed() || other.isArmed()) && ((!defender.isDefensiveUnit() && other.isDefensiveUnit()) || ((!defender.isDefensiveUnit() || other.isDefensiveUnit()) && defenderPower < otherPower)));
	}

	/**
	 * Finds the closest <code>Location</code> to this tile where this ship can be
	...
	public enum MoveType {
		MOVE(null, true), MOVE_HIGH_SEAS(null, true), EXPLORE_LOST_CITY_RUMOUR(null, true), ATTACK_UNIT(null,
				false), ATTACK_SETTLEMENT(null, false), EMBARK(null, false), ENTER_INDIAN_SETTLEMENT_WITH_FREE_COLONIST(
						null, false), ENTER_INDIAN_SETTLEMENT_WITH_SCOUT(null,
								false), ENTER_INDIAN_SETTLEMENT_WITH_MISSIONARY(null,
										false), ENTER_FOREIGN_COLONY_WITH_SCOUT(null,
												false), ENTER_SETTLEMENT_WITH_CARRIER_AND_GOODS(null,
														false), MOVE_NO_MOVES(
	 ...
	 * {@link #getMoveType(Tile, Tile, int)} will return <code>MOVE_NO_MOVES</code>,
	...
	public int getMoveCost(Tile from, Tile target, int ml) {
		...
		if (isBeached(from) || (cost > ml && (ml + 2 >= getInitialMovesLeft() || cost <= ml + 2 || target.hasSettlement()) && ml != 0)) {
	 ...
	 * @param target
	 *            The target <code>Tile</code> of the move.
	 * @return The move type, which will be one of the extended illegal move types
	 *         on failure.
	 */
	 ...
	 * @return True if some sort of legal move to the tile exists, including special
	 *         cases where there is an interaction but the unit does not actually
	 *         move, such as trade.
	 */
	 ...
	 *
	 * @return True if the unit can either move immediately to the high seas or can
	 *         make a move to a neighbouring high seas tile.
	 */
	public boolean hasHighSeasMove() {
		return canMoveToHighSeas() || (hasTile() && getMovesLeft() > 0 && any(getTile().getSurroundingTiles(1, 1), Tile::isDirectlyHighSeasConnected));
	}
	 ...
	 * @return The best entry location tile to arrive on the map at, or null if none
	 *         found.
	 */
	 ...
	 * where the unit will appear when it leaves the high seas, which will either be
	 * Europe or a tile.
	 *
	 * @return The location the unit should appear next after leaving the high seas.
	 */
	...
	public boolean couldMove() {
		return !isDisposed() && getState() == UnitState.ACTIVE && getMovesLeft() > 0 && destination == null // Can not
																											// reach
																											// next tile
	 ...
	 *
	 * Must handle all the cases where the unit is off the map, and take account of
	...
	public Location getPathStartLocation() {
		...
		if (isOnCarrier()) {
			...
			if (getDestination() == null || getDestination() instanceof Map || !(getDestination() instanceof Settlement)) {
	...
	/**
	 * Finds a quickest path between specified locations, optionally using a carrier
	 * and special purpose cost decider.
	 *
	 * @param start
	 ...
	 * @param costDecider
	 *            An optional <code>CostDecider</code> for determining the movement
	 *            costs (uses default cost deciders for the unit/s if not provided).
	 ...
	 * location from its current position. If the unit is currently on a carrier, it
	 * will be used.
	 *
	 * @param end
	 *            The destination <code>Location</code>.
	...
	/**
	 * Gets the number of turns required for this unit to reach a destination
	 * location from a starting location. If the unit is currently on a carrier, it
	 * will be used.
	 *
	 * @param start
	 ...
	 * @param end
	 *            The destination <code>Location</code>.
	 * @return The number of turns it will take to reach the <code>end</code>, or
	 *         <code>MANY_TURNS</code> if no path can be found.
	 */
	...
	/**
	 * Find a path for this unit to the nearest settlement with the same owner that
	 * is reachable without a carrier.
	 *
	 * @param excludeStart
	...
	/**
	 * Find a path for this unit to the nearest settlement with the same owner that
	 * is reachable without a carrier and is connected to Europe by sea, or Europe
	 * if it is closer.
	 *
	 ...
	 *
	 * Used to find where to deliver goods to/from inland colonies, or when blocked.
	 *
	 * @param dst
	 *            The destination <code>Location</code>.
	...
	/**
	 * Find a path for this unit to the nearest settlement with the same owner that
	 * is reachable without a carrier, excepting any on the current tile.
	 *
	...
	/**
	 * Convenience wrapper for the {@link net.sf.freecol.common.model.Map#search}
	 * function.
	 *
	 * @param start
	 *            The <code>Location</code> to start the search from.
	 ...
	 * @param cd
	 *            An optional <code>CostDecider</code> responsible for determining
	 *            the path cost.
	 * @param maxTurns
	 *            The maximum number of turns the given <code>Unit</code> is allowed
	 *            to move. This is the maximum search range for a goal.
	 * @param carrier
	 ...
	 * *can* attack a naval unit if it is beached. Otherwise naval units can only
	 * fight at sea, land units only on land.
	 *
	 * @param defender
	...
	public PathNode searchForDanger(final int range, final float threat) {
		...
		int reverseRange = range * (isNaval() ? getSpecification().getFastestNavalUnitType()
				: getSpecification().getFastestLandUnitType()).getMovement() / getType().getMovement();
	...
	/**
	 * Gets the line of sight of this <code>Unit</code>. That is the distance this
	 * <code>Unit</code> can sight new tiles.
	 *
	 * @return The line of sight of this <code>Unit</code>.
	 */
	public int getLineOfSight() {
		final Turn turn = getGame().getTurn();
		Set<Modifier> result = new HashSet<>(getModifiers(Modifier.LINE_OF_SIGHT_BONUS, unitType, turn));
	 ...
	 * container being null as this can be called in the client on foreign units,
	 * which will not have goods containers.
	 *
	 ...
	 * this unit. Includes empty cargo slots and any spare space in a slot partially
	 * filled with the specified goods.
	 *
	 * @param type
	 ...
	 *
	 * @param player
	 *            The <code>Player</code> viewing the unit, as the owner has access
	 *            to more information.
	 * @param full
	 ...
	 * @param owner
	 *            The <code>Player</code> to own this unit after a change of type
	 *            CAPTURE or UNDEAD.
	 ...
	 * @return The level of skill for the given unit. A higher value signals a more
	 *         advanced type of units.
	 */
	 ...
	 * <code>UnsupportedOperationException</code>, since features can not be added
	 * to Units directly.
	 *
	 * @param feature
	 ...
	 *
	 * A pioneer must be a colonst. Favour: - existing pioneers especially if on the
	 * map - expert pioneer units - then by skill but not other experts
	 *
	 ...
	 *
	 * A scout must be a colonist. Favour: - existing scouts especially if on the
	 * map - expert scouts - lower skill level as scouting is a good career for
	 * crims and servants which might become seasoned scouts
	 *
	 ...
	 *
	 * This can be replaced with just getModifiers() when accepted specifications
	 * have all combat modifiers with correct index values.
	 */
	...
	/**
	 * Get a settlement by identifier, validating as much as possible. Designed for
	 * message unpacking where the identifier should not be trusted.
	 *
	 * @param settlementId
	 ...
	 * Designed for message unpacking where the identifier should not be trusted.
	 *
	 * @param id
	 *            The identifier of the <code>IndianSettlement</code> to be found.
	 * @return The settlement corresponding to the settlementId argument.
	 * @throws IllegalStateException
	 *             on failure to validate the settlementId in any way.
	 */
	 ...
	 * contribute to tile appearance as such, if they move in/out of a colony the
	 * visible colony size changes.
	 *
	 * @param newLocation
	 *            The new <code>Location</code>.
	...
	public StringTemplate getLocationLabel() {
		return StringTemplate.template("model.unit.onBoard").addStringTemplate("%unit%", getLabel());
	}
