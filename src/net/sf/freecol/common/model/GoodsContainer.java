...
public class GoodsContainer extends FreeColGameObject implements Ownable {
	 ...
	 * signficantly bigger than any one unit could expect to carry, but not so huge
	 * as to look silly in user messages.
	 */
	 ...
	 *
	 * Always accessed synchronized and *after synchronized(storedGoods)*. This is
	 * only touched rarely so the extra lock is tolerable. (Not synchronized during
	 * I/O)
	 */
	 ...
	 * @return True if there is enough of the specified goods present that it can be
	 ...
	 * @param type
	 *            The <code>GoodsType</code> being looked for.
	 * @return The amount of this type of goods in this container at the beginning
	 *         of the turn
	 */
	...
	public void removeAbove(int newAmount) {
		synchronized (storedGoods) {
			...
			for (Entry<GoodsType, Integer> entry : storedGoods.entrySet()) {
				...
				if (goodsType.isStorable() && !goodsType.limitIgnored() && entry.getValue() > newAmount) {
	 ...
	 * @param amount
	 *            The amount to check.
	 * @return True if any storable, capacity limited goods has reached the given
	 *         amount.
	 */
	...
	/**
	 * Gets the amount of space that the goods in this container will consume. Each
	 * occupied cargo slot contains an amount in [1, CARGO_SIZE].
	 *
	...
	/**
	 * Has this goods containers contents changed from what was recorded last time
	 * the state was saved?
	 *
	...
	/**
	 * Fire property changes for all goods that have seen level changes since the
	 * last saveState().
	 *
