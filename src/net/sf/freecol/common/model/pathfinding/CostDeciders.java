...
public final class CostDeciders {
	 ...
	 * locations visited when determining cost. Totally ignores the legality of the
	 * move.
	 */
	...
	/**
	 * A <code>CostDecider</code> that only considers the number of tiles visited
	 * when determining the cost, but differs from the trivialCostDecider in
	...
	/**
	 * A <code>CostDecider</code> that uses server-side knowledge of where a player
	 * has explored to limit searches.
	 */
	...
	/**
	 * A <code>CostDecider</code> that costs unit moves normally while avoiding
	 * other player settlements and units, and does not explore if it is trading.
	 */
	private static class AvoidSettlementsAndBlockingUnitsCostDecider extends AvoidSettlementsCostDecider {
		...
		public int getCost(Unit unit, Location oldLocation, Location newLocation, int movesLeft) {
			...
			if (cost != ILLEGAL_MOVE && cost != Map.INFINITY && tile != null) {
				...
				if ((defender != null && defender.getOwner() != unit.getOwner()) || (unit.getTradeRoute() != null && tile.hasLostCityRumour())) {
					return ILLEGAL_MOVE;
				}
			}
			return cost;
	 ...
	 * deciders. The result/s are determined by the cost decider which returns the
	 * highest cost, with an ILLEGAL_MOVE result dominating.
	 *
	 * @param cds
	 *            A series (two minimum) of <code>CostDecider</code>s to compose.
	...
	/**
	 * Selects a default <code>CostDecider</code> for the given unit depending on
	 * the owner of the unit and if the unit can attack other units.
	 *
	 * @param unit
	...
	/**
	 * A <code>CostDecider</code> only considering the number of tiles visited when
	...
	/**
	 * A <code>CostDecider</code> that returns the cost of moving across the
	 * terrain, excluding only illegal moves, and works correctly on the server side
	 * by refusing to consider locations unexplored by the player.
	 *
	 * @return The <code>CostDecider</code>.
	 */
	...
	/**
	 * A <code>CostDecider</code> for avoiding using locations which have blocking
	 * enemy units on them. Paths containing an enemy settlement are considered
	 * illegal, and so are paths where the next move has an enemy unit on it.
	 *
	 * @return The <code>CostDecider</code>.
	 */
	...
	/**
	 * A <code>CostDecider</code> for avoiding using locations which have blocking
	 * enemy units or expose naval units to bombardment.
	 *
	 * @return The <code>CostDecider</code>.
	 */
