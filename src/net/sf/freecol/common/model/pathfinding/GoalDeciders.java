...
public final class GoalDeciders {
	 ...
	 *
	 * @param all
	 *            If true, all deciders must succeed (and-semantics), if false, any
	 *            decide may succeed (or-semantics).
	 * @param gds
	 *            A series (two minimum) of <code>GoalDecider</code>s to compose.
	...
	/**
	 * Gets a GoalDecider to find the `closest' settlement owned by the searching
	 * unit player, with connected ports weighted double.
	 *
	...
	/**
	 * Gets a GoalDecider to find the closest high seas tile to a target. Used when
	 * arriving on the map from Europe.
	 *
		...
		return new GoalDecider() {
			...
			public boolean check(Unit u, PathNode path) {
				...
				if (tile != null && tile.isExploredBy(u.getOwner()) && tile.isDirectlyHighSeasConnected()
						&& (tile.getFirstUnit() == null || u.getOwner().owns(tile.getFirstUnit())) && (best == null || path.getCost() < best.getCost())) {
					best = path;
					return true;
				}
				return false;
	...
	/**
	 * Build a simple goal decider to find the first high seas tile without using
	 * the unit parameter.
	 *
	 ...
	 * @param target
	 *            The target <code>Location</code>.
	 * @return A <code>GoalDecider</code> that only succeeds for tiles adjacent to
	 *         the target location.
	 */
	...
	/**
	 * Get a goal decider that succeeds for settlements owned by one of a given list
	 * of enemies.
	 *
	 * @param enemies
	...
	/**
	 * Goal decider to find the best land tile to disembark a unit that is planning
	 * to attack a given target.
	 *
	 ...
	 * alternatives - Favour a short journey to the target - Prioritize not landing
	 * next to a hostile fort/fortress.
	 *
	 * @param target
	 ...
	 * count than a unit currently has. Useful for tunnelling out of intermittantly
	 * blocked rivers.
	 *
	 * @param unit
	...
	/**
	 * Get a goal decider to find tiles on river corners, preferring ones closest to
	 * the high seas. This is useful when a unit is stuck on a river. By moving to a
	 * corner (i.e. where another unit can get past it) the chance that the blockage
	 * clears is enhanced.
	 *
	...
	/**
	 * A class to wrap a goal decider that searches for paths to an adjacent tile to
	 * a set of locations, and the results of such a search.
	 */
	public static class MultipleAdjacentDecider {
		 ...
		 *
		 * @param locs
		 *            The list of <code>Location</code>s to search for paths to an
