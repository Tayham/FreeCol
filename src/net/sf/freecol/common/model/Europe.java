...
public class Europe extends UnitLocation implements Ownable, Named, TradeLocation {
	 ...
	 * Migration handling.
	 *
	 * Migration routines operate on: - "indexes" which refer to a valid member of
	 * the recruitables list, and must be [0, RECRUIT_COUNT) - "slots", where slot
	 * zero means "pick a random migrant" and the other slots in [1, RECRUIT_COUNT]
	 * refer to the index == slot-1 The following constant should be used when the
	 * random choice behaviour is desired.
	 */
	 ...
	 * @return The price of this unit when trained/purchased in Europe, or UNDEFINED
	 *         on failure.
	 */
	 ...
	 *
	 * Col1 penalizes immigration by -4 per unit in Europe per turn, but there is a
	 * +2 player bonus, which we might as well add here. Total immigration per turn
	 * can not be negative, but that is handled in ServerPlayer.
	 *
	 * @param production
