...
public final class Monarch extends FreeColGameObject implements Named {
	...
	public class Force {
		 ...
		 *
		 * @param naval
		 *            If true, consider only naval units, otherwise consider the land
		 *            units.
	...
	public enum MonarchAction {
		NO_ACTION, RAISE_TAX_ACT, RAISE_TAX_WAR, FORCE_TAX, LOWER_TAX_WAR, LOWER_TAX_OTHER, WAIVE_TAX, ADD_TO_REF, DECLARE_PEACE, DECLARE_WAR, SUPPORT_LAND, SUPPORT_SEA, MONARCH_MERCENARIES, HESSIAN_MERCENARIES, DISPLEASURE;
	 ...
	 * @param player
	 *            The <code>Player</code> to create the <code>Monarch</code> for.
	 */
	...
	/**
	 * Initiates a new <code>Monarch</code> with the given identifier. The object
	...
	/**
	 * Update the intervention force, adding land units depending on turns passed,
	 * and naval units sufficient to transport all land units.
	 */
	...
	public List<AbstractUnit> getSupport(Random random, boolean naval) {
		...
		/*
		 * FIXME: we should use a total strength value, and randomly add individual
		 * units until that limit has been reached. E.g. given a value of 10, we might
		 * select two units with strength 5, or three units with strength 3 (plus one
		 * with strength 1, if there is one).
		 */
