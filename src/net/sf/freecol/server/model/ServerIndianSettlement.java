...
public class ServerIndianSettlement extends IndianSettlement implements ServerModelObject {
	...
	public ServerIndianSettlement(Game game, Player owner, Tile tile, IndianSettlement template) {
		...
		for (Unit unit : template.getUnitList()) {
			Unit newUnit = new ServerUnit(game, this, unit);// -vis: safe, not on map yet
			add(newUnit);
			addOwnedUnit(newUnit);
		}
		Unit missionary = template.getMissionary();
		if (missionary != null) {
			this.missionary = new ServerUnit(game, this, missionary);// -vis: safe not on map
		}
	 ...
	 * units within the ranges provided by the settlement type, otherwise use the
	 * average.
	 *
	 * @param random
	...
	public void csNewTurn(Random random, LogBuilder lb, ChangeSet cs) {
		...
		if (!unitTypes.isEmpty()
			...
			if (ownedUnits.size() <= getType().getMaximumSize()) {
				...
				Unit unit = new ServerUnit(getGame(), getTile(), owner, type);// -vis: safe within settlement
	...
	/**
	 * Updates the most hated nation of this settlement. Needs to be public so it
	 * can be set by backwards compatibility code in FreeColServer.loadGame.
	 *
	 ...
	 * @param propagate
	 *            If true, propagate the alarm change upward to the owning player.
	 * @param cs
	 *            A <code>ChangeSet</code> to update.
	 ...
	 *
	 * +vis: Handles the visibility implications. +til: Handles the tile appearance
	 * change.
	 *
	 * @param missionary
	 ...
	 *
	 * @param destroy
	 *            If true, the settlement is destroyed, if false the missionary is
	 *            denounced, if null do not generate a message.
	 * @param cs
	 *            A <code>ChangeSet</code> to update.
	 */
