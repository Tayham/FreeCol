...
public class ServerColony extends Colony implements ServerModelObject {
	...
	/**
	 * New turn for this colony. Try to find out if the colony is going to survive
	 * (last colonist does not starve) before generating lots of production-related
	 * messages.
	 *
	 * @param random
			...
			if (goodsType == spec.getPrimaryFoodType()) {
				...
				if (net + stored < 0) {
					if (getUnitCount() > 1) {
						...
						cs.addRemove(See.only(owner), null, victim);// -vis: safe, all within colony
						victim.dispose();
	...
	private Unit csBuildUnit(BuildQueue<? extends BuildableType> buildQueue, Random random, ChangeSet cs) {
		...
		Unit unit = new ServerUnit(getGame(), getTile(), owner, type);// -vis: safe, within colony
	...
	/**
	 * Removes a buildable from a build queue, and updates the queue so that a valid
	 * buildable is now being built if possible.
	 *
	 * @param queue
	...
	/**
	 * Evict the users from a tile used by this colony, due to military action from
	 * another unit.
	 *
	 * @param enemyUnit
	 ...
	 *
	 * Triggered by election of laSalle and colony capture by a player with laSalle.
	 *
	 * @param type
