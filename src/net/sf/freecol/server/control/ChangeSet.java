 ...
 * There are a lot of updates
		 ...
		 */
		CHANGE_EARLY(1), CHANGE_NORMAL(15), CHANGE_LATE(90);
	...
	public static class See {
		...
		/**
		 * Make this change visible to all players, provided they can see the objects
		...
		/**
		 * Make this change visible to the given player, provided the player can see the
		 * objects being changed.
		 *
		 * @param player
		 *            a <code>ServerPlayer</code> value
		 * @return a <code>See</code> value
	...
	private static abstract class Change {
		...
		/**
		 * Should a player be notified of a Change for which the visibility is delegated
		 * to the change type, allowing special change-specific overrides.
		 *
		 * This is false by default, subclasses should override when special case
		 ...
		 * @param serverPlayer
		 *            The <code>ServerPlayer</code> to consider.
		 * @return A list of secondary <code>Change</code>s or the empty list if there
		 *         are none, which is usually the case.
		 */
	...
	private static class AttackChange extends Change {
		 ...
		 *
		 * Note that we must copy attackers and defenders because a successful attacker
		 * can move, any an unsuccessful participant can die, and unsuccessful defenders
		 * can be captured. Furthermore for defenders, insufficient information is
		 * serialized when a unit is inside a settlement, but if unscoped too much is
		 * disclosed. So we make a copy and neuter it.
		 *
		 * We have to remember if the defender was in a settlement because by the time
		 * serialization occurs the settlement might have been destroyed.
		 *
		 * We just have to accept that combat animation is an exception to the normal
		 * visibility rules.
		 *
		 * @param see
		...
		/**
		 * Should a player perhaps be notified of this attack? Do not use canSeeUnit
		 * because that gives a false negative for units in settlements, which should be
		 * animated.
		 *
		 * @param serverPlayer
		...
		/**
		 * Specialize a AttackChange into an "animateAttack" element for a particular
		 * player.
		 *
		 * @param serverPlayer
		 *            The <code>ServerPlayer</code> to update.
	...
	private static class MoveChange extends Change {
		...
		/**
		 * There are consequences to a move. If the player can not see the unit after
		 * the move, it should be removed.
		 *
		 * @param serverPlayer
		 *            The <code>ServerPlayer</code> to notify.
		 * @return A RemoveChange if the unit disappears (but not if it is destroyed,
		...
		/**
		 * Specialize a MoveChange into an "animateMove" element for a particular
		 * player.
		 *
		 * @param serverPlayer
		 *            The <code>ServerPlayer</code> to update.
	...
	private static class ObjectChange extends Change {
		 ...
		 * @param doc
		 *            The owner <code>Document</code>.
		 * @return An "update" element, or null if the update should not be visible to
		 *         the player.
		 */
		@Override
		public Element toElement(ServerPlayer serverPlayer, Document doc) {
	...
	private static class RemoveChange extends Change {
		...
		/**
		 * Should a player perhaps be notified of this removal? They should if they can
		 * see the tile, and there is no other-player settlement present.
		 *
		 * @param serverPlayer
		 *            The <code>ServerPlayer</code> to notify.
		 * @return True if the player should be notified.
	...
	/**
	 * Encapsulate trivial element, which will only have attributes apart from its
	 * name.
	 */
	 ...
	 *
	 * @param fcgo
	 *            A <code>FreeColGameObject</code> to remove a matching change for.
	 */
	...
	/**
	 * Helper function to add a dead player event to a ChangeSet. Deaths are public
	 * knowledge.
	 *
	 * @param serverPlayer
	...
	/**
	 * Helper function to add a founding father addition event to a ChangeSet. Also
	 * adds the father to the owner.
	 *
	 * @param serverPlayer
	...
	/**
	 * Helper function to add a global history event to a ChangeSet. Also adds the
	 * history to all the European players.
	 *
	 * @param game
	...
	/**
	 * Helper function to add a history event to a ChangeSet. Also adds the history
	 * to the owner.
	 *
	 * @param serverPlayer
	...
	/**
	 * Helper function to add a new trade route change to a ChangeSet. Also adds the
	 * trade route to the player.
	 *
	 * @param serverPlayer
	 ...
	 * necessarily update its tile correctly on the client side--- if a tile update
