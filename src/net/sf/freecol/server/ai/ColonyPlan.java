...
public class ColonyPlan {
	...
	/**
	 * Lists of goods types to be produced in this colony. Temporary variables that
	...
	/**
	 * Gets the best buildable type from this plan that can currently be built by
	 * the colony.
	 *
	...
	/**
	 * Gets the food-producing and non-autoproducing work location plans associated
	...
	/**
	 * Gets the non-food-producing/non-autoproducing work location plans associated
	 * with this <code>ColonyPlan</code>.
	 *
		...
		for (int i = 0; i < plans.size(); i++) {
			...
			if ((rawBuildingGoodsTypes.contains(g) && !required.contains(g.getOutputType()))
				...
				for (GoodsType type = g.getInputType(); type != null; type = type.getInputType()) {
					...
					if (wls == null || colony.getGoodsCount(type) >= GoodsContainer.CARGO_SIZE / 2) {
						break;
					}
					n += wls.size();
					while (!wls.isEmpty()) {
	...
	/**
	 * Creates a map of potential production of all goods types from all available
	 * work locations using the default unit type. Includes non-workable locations
	 * (e.g. chapel, colony-center-tile) as their production can influence the
	 * choice of goods to produce.
	 *
	 ...
	 * </UL>
	 *
	 * Ignore raw materials which can not be refined and refined goods that have no
	 * raw materials available. Also ignore other goods that do not fit these
	...
	private boolean prioritizeProduction(BuildableType type, GoodsType goodsType) {
		...
		if (goodsType.isMilitaryGoods()) {
			...
			ret = prioritize(type, MILITARY_WEIGHT * factor, 1.0/* FIXME: amount present wrt amount to equip */);
		} else if (goodsType.isBuildingMaterial()) {
			ret = prioritize(type, BUILDING_WEIGHT * factor, 1.0/* FIXME: need for this type */);
			...
			ret = prioritize(type, IMMIGRATION_WEIGHT * factor, 1.0/* FIXME: Brewster? */);
	...
	private void updateBuildableTypes() {
		...
		for (BuildingType type : spec().getBuildingTypeList()) {
			...
			if (type.hasModifier(Modifier.DEFENCE)) {
				...
				prioritize(type, FORTIFY_WEIGHT * factor, 1.0/* FIXME: 0 if FF underway */);
			}
			if (type.hasAbility(Ability.EXPORT)) {
				...
				prioritize(type, EXPORT_WEIGHT * factor, 1.0/* FIXME: weigh production v transport */);
			}
			...
			if (type.hasAbility(Ability.BUILD)) {
				...
				prioritize(type, BUILDING_WEIGHT * factor, support/* FIXME: need for the thing now buildable */);
			}

			if (type.hasAbility(Ability.TEACH)) {
				prioritize(type, TEACH_WEIGHT, 1.0/* FIXME: #students, #specialists here, #wanted */);
			}
			...
			if (type.hasAbility(Ability.REPAIR_UNITS)) {
				...
				prioritize(type, REPAIR_WEIGHT * factor, 1.0/* FIXME: #units-to-repair, has-Europe etc */);
			}
			...
			if (output != null) {
				...
				if (type.hasModifier(Modifier.WAREHOUSE_STORAGE)) {
					...
					prioritize(type, STORAGE_WEIGHT * factor, 1.0/* FIXME: amount of goods */);
				}
				if (type.hasModifier(Modifier.BREEDING_DIVISOR)) {
					prioritize(type, BREEDING_WEIGHT, 1.0/* FIXME: horses present? */);
				}
			}
			...
			if (!unitType.hasAbility(Ability.NAVAL_UNIT)) {
				if (unitType.isDefensive()) {
					if (colony.isBadlyDefended()) {
						prioritize(unitType, DEFENCE_WEIGHT, 1.0/* FIXME: how badly defended? */);
					}
					...
					prioritize(unitType, TRANSPORT_WEIGHT * factor, wagonNeed/* FIXME: type.getSpace() */);
				}
			}
		}
	 ...
	 * locations that can use a unit or can auto-produce. Note that this will almost
	 * certainly include clashes over work locations. That gets sorted out elsewhere
	 * as ColonyPlans do not examine the units present.
	 *
	 * With the complete list of work plans, finish creating the list of goods to
	 * produce.
	 *
	 * Then filter out the auto-production plans as they are not going to be helpful
	 * for unit allocation.
	 *
	...
	/**
	 * Add the other goods types to the production list. When this is called the new
	 * world goods production is already present on the produce list. Ignores food
	 ...
	 * @return The first plan found that produces the goods type, or null if none
	 *         found.
	 */
	...
	/**
	 * Gets the best worker to execute a work location plan. - The most productive
	 * one wins (which will automatically pick a relevant expert). - If they are all
	 * relevant experts, pick any. - Pick the unit that can upgrade to the required
	 * expert with the most relevant experience or least irrelevant expertise. -
	 * Pick a unit that can not upgrade at all. - Pick an otherwise upgradeable unit
	 * with the most relevant experience or least irrelevant experience. - Pick the
	 * least skillful unit.
	 *
	 ...
	 * No i18n here, this is for debugging purposes.
	 *
	 * @param loc
