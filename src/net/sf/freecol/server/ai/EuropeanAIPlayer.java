...
public class EuropeanAIPlayer extends AIPlayer {
	 ...
	 * low-ish because it is usually more efficient to ship the tools where they are
	 * needed and either create a new pioneer on site or send a hardy pioneer on
	 * horseback. The AI is probably smart enough to do the former already, and one
	 * day the latter.
	 */
	 ...
	 *
	 * We do not check if a unit is near to a colony that can provide tools, as that
	 * is likely to be too expensive. FIXME: perhaps we should.
	 */
	...
	/**
	 * A mapping of goods type to the goods wishes where a colony has requested that
	 * goods type. Used to retarget goods that have gone astray.
	 */
	private final java.util.Map<GoodsType, List<GoodsWish>> goodsWishes = new HashMap<>();

	/**
	 * A mapping of unit type to the worker wishes for that type. Used to allocate
	 * WishRealizationMissions for units.
	 */
	 ...
	 *
	 * FIXME: European players can also bring gifts! However, this might be folded
	 * into a trade mission, since European gifts are just a special case of
	 * trading.
	 *
	 * @param lb
	 *            A <code>LogBuilder</code> to log to.
	 ...
	 *
	 * Called by startWorking at the start of every turn. Public for the test suite.
	 *
	 * @param lb
	 *            A <code>LogBuilder</code> to log to.
	 */
	 ...
	 *
	 * If the target has changed - drop all non-boarded transport unless the target
	 * is the same - dump boarded transport with no target - requeue all boarded
	 * transport unless the target is the same
	 *
	 * @param aiu
	...
	/**
	 * Changes the needed wagons map for a specified tile/contiguity. If the change
	 * is zero, that is a special flag that a connected port is available, and thus
	 * that the map should be initialized for that contiguity.
	 *
	 * @param tile
	 *            The <code>Tile</code> to derive the contiguity from.
	 * @param amount
	...
	/**
	 * Allows a TransportMission to signal that it has taken responsibility for a
	 ...
	 *
	 * This is the desired total number, not the actual number which would take into
	 * account the number of existing BuildColonyMissions.
	 *
	 ...
	 *
	 * This is the desired total number, not the actual number which would take into
	 * account the number of existing PioneeringMissions.
	 *
	 ...
	 *
	 * This is the desired total number, not the actual number which would take into
	 * account the number of existing ScoutingMissions.
	 *
	...
	/**
	 * Helper function for server communication - Ask the server to train a unit in
	 * Europe on behalf of the AIGetPlayer().
	 *
	 * FIXME: Move this to a specialized Handler class (AIEurope?) FIXME: Give
	 * protected access?
	 *
	...
	/**
	 * Is this player lagging in naval strength? Calculate the ratio of its naval
	 * strength to the average strength of other European colonial powers.
	 *
	 * @return The naval strength ratio, or negative if there are no other European
	 *         colonial nations.
	 */
	...
	protected Stance determineStance(Player other) {
		final Player player = getPlayer();
		return other.isREF() ? ((player.getREFPlayer() == other)
	...
	public TradeStatus acceptDiplomaticTrade(DiplomaticTrade agreement) {
		...
		if (result == null && randomInt(logger, "Enough diplomacy?", getAIRandom(), 1 + agreement.getVersion()) > 5) {
			result = rejectAgreement(peace, agreement);
