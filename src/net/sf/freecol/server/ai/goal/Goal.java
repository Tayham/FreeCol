...
public abstract class Goal extends AIObject implements GoalConstants {
	...
	/**
	 * Alternate constructor - directly add a unit to this Goal. The calling object
	 * ensures that this unit is not currently part of another Goal.
	 *
	 * @param p
	 *            The {@link AIPlayer} this goal belongs to
	 * @param g
	 ...
	 * object calling this. After this method has been called, it should be safe for
	 ...
	 * Should be called by the {@link AIPlayer} once for each of its subgoals at the
	 * start of a turn. The Goal will handle all other instances of this flag
	 * needing to be reset internally.
	 *
	 * @param p
	 *            Boolean determining whether to set needsPlanning =true or =false
	 */
	 ...
	 *
	 * @return The absolute weight [0;1] of the parent goal, or 1 if a parent goal
	 *         does not exist
	 */
	 ...
	 * {@link AIPlayer}. This is used when making requests, to allow the AIPlayer to
	 * find the "most important" request.
	 *
	 ...
	 * goal. The parent should assure that the sum of weights given to its subgoals
	 * is =1
	 *
	 * @param w
	...
	// * of this goal, and the number of turns since a unit request has last been
	// granted.
	 ...
	 * </p>
	 * <p>
	 * Each Goal can request necessary units from the AIPlayer. Here, such a request
	 * is wrapped in a private method for convenience. Each request contains a
	 * weight, which is {@link #getAbsoluteWeight()} of this goal, and the number of
	 * turns since a unit request has last been granted. The latter should be taken
	 * into account as a "bonus weight" by the AIPlayer.
	 * </p>
	 * <p>
	 * FIXME: AIPlayer#addUnitWish(Goal,GoodsType,int,float,int); should add
	 * requests to a set-like structure, so that there's only one active request per
	 * Goal at any time. Since fulfilling a request using {@link #addUnit(AIUnit)}
	 * means that {@link #plan()} will be called again during the turn, the Goal
	 * will be able to request again.
	 *
	 * @param gt
	...
	/**
	 * Adds a unit to this goal. This may be from {@link AIPlayer} fulfilling a unit
	 * request, by the parent goal, or by a subgoal that no longer needs the unit.
	 * </p>
	 * <p>
	 * Possible FIXME: If the unit we're requesting is a high-interest one, such as
	 * a Galleon, AIUnit#setLoaningGoal() may be used to signal that this unit may
	 * _only_ be moved to subgoals, or back to {@link AIPlayer}, but not further up
	 * the hierarchy or to any other requesting Goal.
	 *
	 * @param u
	...
	/**
	 * Adds a unit to the parent goal. If this goal doesn't have a parent goal, the
	 * unit will be added to {@link AIPlayer} instead.
	 *
	 * @param u
	 ...
	 * @param o
	 *            The {@link AIObject} (should be AIPlayer or another Goal) calling
	 *            this
	 ...
	 *         that would be yielded. Note that the returned value might be 99f if
	 *         there is no unit to yield. The calling function should use
	 *         {@link #canYieldUnit(UnitType,AIObject)} first, or is responsible to
	 *         sanitize this result itself before trying to
	...
	/**
	 * Removes a unit from the goal, potentially from a subgoal, and yields it to
	 * the caller.
	 * </p>
	 * <p>
	 ...
	 *            The {@link AIObject} (should be AIPlayer or another Goal) calling
	 *            this
	 * @return The {@link AIUnit} with minimal absolute weight. Note that this may
	 *         be null if no matching unit is found!
	 */
	...
	/**
	 * Checks all owned AIUnits for validity, and removes invalid ones. An AIUnit is
	 * supposed to be invalid if it no longer contains a valid Unit. This may be the
	 * case if the Unit has been removed from the game between turns.
	 * </p>
	 * <p>
	 * NOTE: The assumption here is that AIUnit#isValid() will return true as long
	 * as the {@link net.sf.freecol.common.model.Unit} wrapped in it exists.
	 */
	...
	/**
	 * Ensures that a unit moved to another Goal is properly removed from this. If a
	 * unit is removed from this goal via {@link #yieldUnit(UnitType,AIObject)}, or
	 * if the Unit contained in the {@link AIUnit} no longer is valid, this method
	 * is called to clean up any remaining references to the unit.
	 * </p>
	 * <p>
	 ...
	 * <li>last but not least, by spending their movement points for some internal
	 * mission</li>
	 * </ul>
	 * </li>
	 ...
	 *
	 * Assuming the AIPlayer has a set of goals... At the start of a turn, it will
	 * call
	 *
	 ...
	 *
	 * boolean furtherPlanning = true; while (furtherPlanning) { FOR ALL GOALS g DO
	 * { g.doPlanning(); }
	 *
	 ...
	 *
	 * The first FOR-loop will recursively reach all existing goals and plan() each
	 * of them once, bottom-up.
	 *
	 * After that, unit requests from the goals will have piled up.
	 * SATISFY_UNIT_REQUESTS() will try to satisfy as many as possible, in order of
	 * importance, by using the units that were created between turns or that have
	 * been given back by other goals.
	 *
	 * HANDLE_REMAINING_REQUESTS() will try to do something about remaining
	 * requests, for example by buying/building new units, setting up new goals etc.
	 *
	 * This, as well as goals called later during this process moving a unit to a
	 * former goal, may have set one or more of the goals to needsPlanning=true. In
	 * the second FOR-loop, we check whether this is the case, to eventually repeat
	 * the process.
	 *
	 * Eventually, the WHILE-loop will exit. Any units in the goals will have been
	 * used for this turn. There may still be some excess units that haven't been
	 * requested by any Goal. The AIPlayer may now choose to create a new Goal for
	 * these, add them to an existing Goal (which will make use of them next turn),
