...
public class IndianDemandMission extends Mission {
	...
	private int capAmount(int amount, int difficulty) {
		return Math.min(Math.max(amount * difficulty / 6, GOODS_DEMAND_MIN), GoodsContainer.CARGO_SIZE); // One load of
																											// goods max
	}
	 ...
	 * @return A reason why the mission would be invalid with the unit and colony or
	 *         null if none found.
	 */
