...
public class TransportMission extends Mission {
	...
	/**
	 * Decide if this unit has a good chance of defeating another. If there is cargo
	 * aboard, be more conservative.
	 *
	...
	/**
	 * How many more destinations are desirable on the current cargoes list? Must be
	 * public! This is checked in European AI player.
	 *
	...
	/**
	 * If this carrier is the not the current carrier of a transportable, make it
	 * so.
	 *
	 * @param t
	 *            The <code>TransportableAIObject</code> to check.
	 ...
	 *
	 * Public so that mobile transportables (units) can renege on transport if they
	 * find themselves better able to get there themselves.
	 *
	 * @param t
	 *            The <code>TransportableAIObject</code> to collect.
	...
	/**
	 * For a given transportable, work out where the carrier has to go to advance
	 * the cargo (target), and what to do there (mode), allowing a new
	 ...
	 * Otherwise try to place it with other cargoes with the same target, but do not
	 * break the space restrictions. If this does not work, it has to go at the end.
	 *
	 * @param cargo
	 ...
	 * cargoes list. On exit from this routine, every cargo on board should be on
	 * the cargoes list but the list is not necessarily going to be in a sensible
	 * order.
	 *
	 * @param lb
	 *            A <code>LogBuilder</code> to log to.
	 ...
	 * @param lb
	 *            A <code>LogBuilder</code> to log to.
	 * @return TCONTINUE if the <code>Cargo</code> should continue, TDONE if it has
	 *         completed, TFAIL if it has failed, TNEXT if it has progressed to the
	 *         next stage, TRETRY if a blockage has occurred and it should be
	 *         retried,
	 */
	...
	/**
	 * Perform the transport load/unload operations on arrival at the target for the
	 * top cargo.
	 *
	 * @param lb
	 *            A <code>LogBuilder</code> to log to.
	 ...
	 * primarily on the number of turns it takes, but to break ties we also consider
	 * the hold*turn product to reduce the risk of losses due to enemy action.
	 *
	 ...
	 * @return A reason why the mission would be invalid with the unit, or null if
	 *         none found.
	 */
	 ...
	 * locations. A location becomes invalid if: - step is a null location - step is
	 * disposed - step is a captured settlement
	 *
	 * @param cargo
	 ...
	 * EuropeanAIPlayer.allocateTransportables and CashInTreasureTrain.doMission.
	 *
	 * @param t
	 ...
	 * @param lb
	 *            A <code>LogBuilder</code> to log to.
	 * @return True if the transportable is no longer on board, queued, or was reset
	 ...
	 * Useful for prioritizing treasure collection.
	 *
	 * @param aiu
