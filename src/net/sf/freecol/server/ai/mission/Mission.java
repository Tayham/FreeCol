...
public abstract class Mission extends AIObject {
	 ...
	 *
	 * Note that missions are attached to their units, and thus do not need AI ids,
	 * hence the plain superclass constructor.
	 *
	 * @param aiMain
	 *            The main AI-object.
	 * @param aiUnit
	 ...
	 * non-onetime mission?
	 *
	 * @return "mission-exists" if a valid mission is found, or null if none found.
	 */
	 ...
	 *
	 * AIPlayers will call FooMission.invalidReason(aiUnit) to determine whether it
	 * is valid to assign some unit to a FooMission, so `interesting' Mission
	 * subclasses with complex validity requirements must implement a routine with
	 * this signature. Conversely, simple Missions that are always possible need
	 * not.
	 *
	 ...
	 *
	 * Specific Missions can be invalid for target-related reasons. Such Missions
	 * need to implement a routine with this signature, as it will be called by the
	 * GoalDeciders in map path find/searches to choose a Mission target.
	 *
	 * Implementations should usually start by calling either invalidAIUnitReason()
	 * or this routine if the target checking is trivial.
	 *
	 * @param aiUnit
	 *            The <code>AIUnit</code> to check.
	 * @param loc
	 ...
	 *            A <code>LogBuilder</code> to log to.
	 * @param cont
	 *            If true, the mission should continue, otherwise drop the mission.
	 * @param reasons
	...
	/**
	 * Finds a target for a unit without considering its movement abilities. This is
	 * used by missions when the current unit can not find a target with the normal
	 * path finding routines, and thus should consider targets that may require a
	 * carrier.
	 *
	 * @param aiUnit
	...
	/**
	 * Finds the best existing settlement to use as a target. Useful for missions
	 * where the unit might be in Europe, but should go to a safe spot in the New
	 * World and proceed from there.
	 *
	 * @param player
	 ...
	 *
	 * First check for units in transit, that is units on a carrier that are going
	 * to but not yet in Europe, or going to but not yet at a Tile and whose path
	 * still requires the carrier. These need to be handled by the carrier's
	 * TransportMission, not by the unit's Mission.
	 *
	 * Similarly check for units not in transit but should be, that is units not on
	 * a carrier but can not get to their target without one. These must just wait.
	 *
	 ...
	 * MOVE_HIGH_SEAS if the unit has set sail to/from Europe - MOVE_NO_MOVES is
	 * underway but ran out of moves - MOVE_NO_ACCESS_EMBARK if progress depends on
	 * a carrier, either currently boarded or due to collect the unit -
	 * MOVE_NO_REPAIR if the unit died for whatever reason - MOVE_NO_TILE if there
	 * is no path (usually transitory on rivers) - MOVE_ILLEGAL if there is an error
	 * or permanent restriction - other legal results (e.g.
	...
	/**
	 * Follow a path that is on the map (except perhaps the last node) and does not
	 * use a carrier.
	 *
	 * @param path
	 ...
	 *
	 * Override this in the child mission classes if there is a useful intermediate
	 * point to deliver the unit distinct from the target.
	 *
	 * @return The mission target, or null if the mission is invalid, otherwise
	 *         lacks a target (e.g. UnitWanderHostile), or the unit does not need
	 *         transport.
	 */
	 ...
	 *
	 * Missions are not one-time by default, true one-time missions must override
	 * this routine.
	 *
	 * @return False.
	 */
	 ...
	 *
	 * Mission subclasses must implement this routine, which probably should start
	 * by checking invalidAIUnitReason.
	 * 
	 * A mission can be invalid for a number of subclass-specific reasons. For
	 * example: a seek-and-destroy mission could be invalid because of a improved
	 * stance towards the targeted player.
	 *
	 * @return A reason for mission invalidity, or null if none found.
	 */
