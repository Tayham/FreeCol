...
public class UnitSeekAndDestroyMission extends Mission {
	 ...
	 *
	 * Do not cheat and look inside the settlement. Just use visible facts about it.
	 *
	 * FIXME: if we are the REF and there is a significant Tory population inside,
	 * assume traitors have briefed us.
	 *
	 * @param aiUnit
	...
	private static int scoreUnitPath(AIUnit aiUnit, PathNode path, Unit defender) {
		...
		} else if (defender.hasAbility(Ability.EXPERT_SOLDIER) && !defender.isArmed()) {
			value += 100;
		}
		return aiUnit.getAIOwner().adjustMission(aiUnit, path, UnitSeekAndDestroyMission.class, value);
	}
