...
public class PioneeringMission extends Mission {
	 ...
	 * PioneeringMission.isValid(aiUnit) should be called before this, to guarantee
	 * that findTileImprovementPlan/findColonyWithTools succeed.
	 *
	 * @param aiMain
	 *            The main AI-object.
	 * @param aiUnit
	...
	/**
	 * Get the best improvement associated with a tile for a given unit. Take care
	 * to first check if the unit has a plan already, if so, return that.
	 *
	 * @return The <code>TileImprovementPlan</code>, or null if not found.
	 */
	...
	/**
	 * Sets the <code>TileImprovementPlan</code> which should be the next target.
	 *
	 * @param tip
	...
	/**
	 * Finds a suitable pioneering target for the supplied unit. Falls back to the
	 * best settlement if the unit is not on the map.
	 *
	 * @param aiUnit
	 ...
	 * @return A reason why the unit can not perform this mission, or null if none.
	 */
	...
	public Mission doMission(LogBuilder lb) {
		...
		/*
		 * if (retarget) { newTarget = findTarget(aiUnit, 10, unit.isInEurope()); if
		 * (newTarget == null) { setTarget(null); return lbFail(lb, false,
		 * " found no target"); } setTarget(newTarget); lb.add(", retargeting ",
		 * newTarget, "/", tileImprovementPlan); }
		 */
		 ...
		 * null) { turnsNeeded = Math.min(turnsNeeded, unit.getWorkLeft()); } if
		 * (unit.isInDanger(turnsNeeded, 0.25f)) { if (unit.getTile().getColony() !=
		 * null) { logger.finest(tag + " avoiding danger: " + this); return lbDrop(lb);
		 * } PathNode safe = unit.findOurNearestSettlement(false, 1, false); if (safe !=
		 * null) { travelToTarget(tag + " (evading)", safe.getLastNode().getTile(),
		...
		if (!player.owns(tile)) {
			...
			if (price < 0) {
				...
				fail = !AIMessage.askClaimLand(tile, aiUnit, price) || !player.owns(tile); // Failed to claim ownership
			}
			if (fail) {
