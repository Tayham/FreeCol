...
public class REFAIPlayer extends EuropeanAIPlayer {
	...
	/**
	 * Initialize the REF. - Find the initial target - Give valid missions to all
	 * the units
	 *
	 * Note that we can not rely on normal AI processing as the "teleporting"
	 * Col1-style REF needs to be placed on the map before its first turn starts, so
	 * the server should ask this AI where it should arrive on the map.
	 *
	 * Note also that to find a target we can not just call getMilitaryMission and
	 * aim for it as the getMilitaryMission scoring includes distance from starting
	 * point, which is what we are trying to determine. So, just choose the best
	 * coastal colony.
	 *
	 * FIXME: Mission assignment is done here because ATM the European AI is prone
	 * to send ships full of troops off to attack the rebel navy. If this is fixed
	 * check if the normal mission assignment works and drop it from here.
	 *
	 * @param teleport
