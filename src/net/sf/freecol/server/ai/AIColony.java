...
public class AIColony extends AIObject implements PropertyChangeListener {
	...
	/**
	 * Do not perform tile improvements that would leave less than this amount of
	 * forested work locations available to the colony.
	 */
	 ...
	 * disposition of export goods in this colony - tile improvements (might ignore
	 * freshly grabbed tiles) - wishes
	 *
	 * @param lb
	...
	public Collection<AIUnit> rearrangeWorkers(LogBuilder lb) {
		...
		for (Unit u : tile.getUnitList()) {
			...
			if ((u.hasAbility(Ability.SPEAK_WITH_CHIEF) && (m = aiPlayer.getScoutingMission(aiu)) != null) || (u.isDefensiveUnit() && (m = aiPlayer.getDefendSettlementMission(aiu, colony)) != null) || (u.hasAbility(Ability.ESTABLISH_MISSION) && (m = aiPlayer.getMissionaryMission(aiu)) != null)) {
				lb.add(", ", m);
				aiPlayer.updateTransport(aiu, oldTarget, lb);
				continue;
			}
	 ...
	 * everything if at war with the owner, otherwise just take the tile that best
	 * helps with the currently required raw building materials, with a lesser
	 * interest in food.
	 *
	 * @param lb
	 *            A <code>LogBuilder</code> to log to.
	 ...
	 *
	 * Throwing an exception stalls the AI and wrecks the colony in a weird way. Try
	 * to recover by hopefully finding a unit outside the colony and stuffing it
	 * into the town hall.
	 *
	 ...
	 *
	 * Called from BuildColonyMission to clear a colony tile that is about to have a
	 * colony built on it.
	 *
	 * @param wl
	...
	/**
	 * Removes the given <code>AIGoods</code> from the export goods for this colony.
	 * The <code>AIGoods</code>-object is not disposed as part of this operation.
	 * Use dropExportGoods instead to remove the object completely (this method
	...
	private void goodsLog(AIGoods ag, String action, LogBuilder lb) {
		...
		lb.add(", ", action, " ", (ag == null) ? "(null)" : ag.getId(), " ",
				(amount >= GoodsContainer.CARGO_SIZE) ? "full " : amount + " ", type);
	}
	 ...
	 * colony. If one is already present, the amount specified here takes precedence
	 * as it is more likely to be up to date. The value is treated as a minimum
	 * requirement.
	 *
	 * @param type
	...
	/**
	 * Requires a worker wish for a unit type to be provided to this AIColony. If a
	 * suitable wish is already present, the expert and value parameters take
	 * precedence as they are more likely to be up to date.
	 *
	 * @param type
	...
	/**
	 * Creates a list of the <code>Tile</code>-improvements which will increase the
	 * production by this <code>Colony</code>.
	 *
