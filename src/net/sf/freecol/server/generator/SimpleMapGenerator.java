...
public class SimpleMapGenerator implements MapGenerator {
	 ...
	 * indicating how much of the half map close to the pole cannot be spawned on.
	 */
	...
	private void recache(boolean checkImport) {
		...
		File importFile = checkImport
				? ((FileOption) this.mapOptions.getOption(MapGeneratorOptions.IMPORT_FILE)).getValue()
				: null;
	...
	/**
	 * Make the native settlements, at least a capital for every nation and random
	 * numbers of other settlements.
	 *
	 * @param map
	...
	/**
	 * Is a tile suitable for a native settlement? Require the tile be settleable,
	 * and at least half its neighbours also be settleable.
	 *
	...
	/**
	 * Create two ships, one with a colonist, for each player, and select suitable
	 * starting positions.
	 *
	 * @param map
