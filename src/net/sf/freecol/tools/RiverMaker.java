...
public class RiverMaker {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int index = 1; index < 81; index++) {
			 ...
			 * Path2D.Float path = new Path2D.Float(); float dx = (NW.x - SE.x)/6; float dy
			 * = (NW.y - SE.y)/6; path.moveTo(NW.x, NW.y); //path.lineTo(SE.x, SE.y);
			 * path.lineTo(SE.x - 16, SE.y - 8); path.quadTo(BASE_WIDTH - 16, HALF_HEIGHT,
			 * SE.x - 16, SE.y + 8); path.lineTo(SW.x, SW.y); //path.moveTo(NE.x, NE.y);
